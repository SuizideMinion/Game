...
//schauen wie lange der l�ngste spieler dabei ist
$db_daten=mysql_query("SELECT MAX(tick) as maxtick FROM de_user_data",$db);
$row = mysql_fetch_array($db_daten);
$maxtick=$row["maxtick"];

//wenn die ticks rum sind siegelfunktion durchf�hren und kollektoren abziehen
if ($siegel1>=getDefaultVariable('sv_siegel1')[1]-1)
{
  ...
  while ($row = mysql_fetch_array($db_daten))
  {
    ...
    $p=$col/10000;
    if($p>getDefaultVariable('sv_siegel1')[2])$p=getDefaultVariable('sv_siegel1')[2];
    ...
    if ($colverlust>0)
    {
      ...
      if($colverlust==1) $text="Die Schockwelle, ausgel�st durch das Siegel von Basranur, durcheilt den Hyperraum und trifft auf Ihr System. Sie verlieren einen Kollektor.";
      else $text="Die Schockwelle, ausgel�st durch das Siegel von Basranur, durcheilt den Hyperraum und trifft auf Ihr System. Sie verlieren $colverlust Kollektoren.";
...
//schauen ob man das siegel hochz�hlen mu�
elseif($maxtick>getDefaultVariable('sv_siegel1')[0])
{
  //siegel hochz�hlen
...
//wenn es spieler gibt und f�r jeden genug npcs vorhanden sind die quest in der db eintragen
if ($pc>0 AND $npc>=$pc)
{
  //als allererstes aber mal die alten eintr�ge l�schen
  ...
  while ($pc = mysql_fetch_array($db_pc))
  {
    ...
    //quest nur dann ins system einf�gen, wenn pc und npc �ber eine gewisse non-fixpunktzahl verf�gen
    //aber genaues wei� man da eh erst sp�ter, daher erstmal einach simpel die sachen einf�gen


    //evtl nur dann ne quest starten, wenn alle npcs nen bestimmten softscore wert haben, wobei das auch vieles wieder verz�gern kann
