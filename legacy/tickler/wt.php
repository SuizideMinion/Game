...
for ($rasse=1; $rasse<=getDefaultVariable('sv_anz_rassen'); $rasse++){
	...
	while($irow = mysql_fetch_array($res)){
		...
		if((($maxtick>2500000 && getDefaultVariable('sv_ewige_runde')!=1 && $sv_hardcore!=1) OR ($sv_comserver==1 AND $sv_comserver_roundtyp==1)) AND $sector>1){
		...
		if ($zufallverteilen>0 and $tick>getDefaultVariable('sv_global_start_zufall')){
		  ...
		  if ($zcount==1 AND $sector > 1 AND $npc==0){
			...
			include $directory."inc/lang/".getDefaultVariable('sv_server_lang')."_wt_zufallmsg.lang.php";
...
while($irow = mysql_fetch_array($res)){
	...
	//wert berechnen
	if(isset(getDefaultVariable('sv_ewige_runde')) && getDefaultVariable('sv_ewige_runde')==1){
		//echo '<br>'.$uid.':'.$sector;
