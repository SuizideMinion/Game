...
function showMessage($key, $count, $material = null)
{
    $message = Lang::get('wt.'. $key .'.0', [
        'count' => number_format($count, 0, ',', '.'),
        'name' => $material,
    ]);

    return $message; // Gibt die vollständige Nachricht zurück
}
...
include $directory . "inc/lang/" . getDefaultVariable('sv_server_lang') . "_wt.lang.php";
...
//spieler umziehen, die zu gro� f�r den startsektor sind, au�er die funktion ist deaktiviert
    if (getDefaultVariable('sv_deactivate_sec1moveout') == 0) {
...
//wenn $globtcount null dann schauen ob man die t-verteilung nochmal anstößt
    ...
    if ($globtcount == 0) {
        ...
        if ($r < getDefaultVariable('sv_globalw_tronic')) {
            $tronicverteilen = 1;
        } else {
            $tronicverteilen = 0;
        }
...
//wenn $globzcount null dann schauen ob man die Zufalls-verteilung nochmal anst��t
    ...
    if ($globzcount == 0) {
        ...
        if ($r < getDefaultVariable('sv_globalw_zufall')) $zufallverteilen = 1;
...
////////////////////////////////////////////////
    for ($rasse = 1; $rasse <= getDefaultVariable('sv_anz_rassen'); $rasse++) {
        echo '<br>Rasse: ' . $rasse;
        ...
        include $directory . "inc/lang/" . getDefaultVariable('sv_server_lang') . "_wt_tronicmsg.lang.php";
        ...
        while ($irow = mysql_fetch_array($res)) {
            ...
            //if($useefta==1)$malus=getDefaultVariable('sv_efta_col_malus');else $malus=0;
            $malus = 0;
            $sabotagemalus = 0;
            ...
            /*
		if($premium==0){
			$ea=$col*(getDefaultVariable('sv_kollieertrag')-$malus-$sabotagemalus);
		}else{
			$ea=$col*(getDefaultVariable('sv_kollieertrag_pa')-$malus-$sabotagemalus);
		}*/
            ...
            $eartefaktenergie = getDefaultVariable('sv_eftaartefaktertrag') * $eartefakt;

            //kriegsartefakt
            //$kartefaktenergie=floor($ea/1000*$kartefakt);
            $kartefaktenergie = getDefaultVariable('sv_kriegsartefaktertrag') * $kartefakt;
            ...
            $pa_energie = $maxcol * getDefaultVariable('sv_kollieertrag') * 0.05;
            ...
            if ((($maxtick > 2500000 && getDefaultVariable('sv_ewige_runde') != 1 && getDefaultVariable('sv_hardcore') != 1) or (getDefaultVariable('sv_comserver') == 1 and getDefaultVariable('sv_comserver_roundtyp') == 1)) and $sector > 1) {
            ...
            if (!hasTech($pt, 4)) {//keine gilde
                $grundm = getDefaultVariable('sv_plan_grundertrag')[0] * $grundertragmultiplikator * (1 + $spec3cache[$sector]);
                $grundd = getDefaultVariable('sv_plan_grundertrag')[1] * $grundertragmultiplikator * (1 + $spec3cache[$sector]);
                $grundi = getDefaultVariable('sv_plan_grundertrag')[2] * $grundertragmultiplikator * (1 + $spec3cache[$sector]);
                $grunde = getDefaultVariable('sv_plan_grundertrag')[3] * $grundertragmultiplikator * (1 + $spec3cache[$sector]);
            } else {  //mit gilde
                $grundm = getDefaultVariable('sv_plan_grundertrag_whg')[0] * $grundertragmultiplikator * (1 + $spec3cache[$sector]);
                $grundd = getDefaultVariable('sv_plan_grundertrag_whg')[1] * $grundertragmultiplikator * (1 + $spec3cache[$sector]);
                $grundi = getDefaultVariable('sv_plan_grundertrag_whg')[2] * $grundertragmultiplikator * (1 + $spec3cache[$sector]);
                $grunde = getDefaultVariable('sv_plan_grundertrag_whg')[3] * $grundertragmultiplikator * (1 + $spec3cache[$sector]);
            }

            $grundm = $grundm + floor($agent_lost * getDefaultVariable('sv_zoellnerertrag')[0]);
            $grundd = $grundd + floor($agent_lost * getDefaultVariable('sv_zoellnerertrag')[1]);
            $grundi = $grundi + floor($agent_lost * getDefaultVariable('sv_zoellnerertrag')[2]);
            $grunde = $grunde + floor($agent_lost * getDefaultVariable('sv_zoellnerertrag')[3]);
            ...
            if ($zufallverteilen > 0 and $tick > getDefaultVariable('sv_global_start_zufall')) {
                ...
                if ($zcount == 1 and $sector > 1 and $npc == 0) {
                    ...
                    switch ($r) {
                        case 0:
                            ...
                            $nachricht = __('wt.kolliemsg.' . rand(0, countLangEntries('wt.kolliemsg')), [
                                'count' => number_format($zufallskollies, 0, ',', '.')
                            ]);
                            break;
                        ...
                        case 1:
                            ...
                            $nachricht = __('wt.schiffmsg.' . rand(0, countLangEntries('wt.schiffmsg')), [
                                'count' => number_format($anzahl, 0, ',', '.')
                            ]);
                            break;
                        ...
                        case 2:
                            ...
                            $nachricht = __('wt.deffmsg.' . rand(0, countLangEntries('wt.deffmsg')), [
                                'count' => number_format($zufallsturm, 0, ',', '.')
                            ]);
                            break;
                        ...
                        case 3:
                            ...
                            $nachricht = __('wt.erfahrungsmsg.' . rand(0, countLangEntries('wt.erfahrungsmsg')), [
                                'count' => number_format($zufallerfahrung, 0, ',', '.')
                            ]);
                            break;
                        ...
                        case 4:
                            ...
                            $nachricht = __('wt.agentenmsg.' . rand(0, countLangEntries('wt.agentenmsg')), [
                                'count' => number_format($zufallsagenten, 0, ',', '.')
                            ]);
                            break;
                        ...
                        case 5:
                            ...
                            $nachricht = __('wt.sondenmsg.' . rand(0, countLangEntries('wt.sondenmsg')), [
                                'count' => number_format($zufallssonden, 0, ',', '.')
                            ]);
                            break;
                        ...
                        case 6:
                            ...
                            $nachricht = __('wt.ressmsg.' . rand(0, countLangEntries('wt.ressmsg')), [
                                'count' => number_format($anzahl, 0, ',', '.')
                            ]);
                            break;
                    }
            ...
            for ($p = 0; $p < getDefaultVariable('sv_anz_schiffe'); $p++) {
            ...
            for ($p = 0; $p < getDefaultVariable('sv_anz_tuerme'); $p++) {
                $punkte = $punkte + $ec[$p + 100] * $unit[$rasse - 1][$p + getDefaultVariable('sv_anz_schiffe')][4];
            }
...
while ($row = mysql_fetch_array($db_daten))
{
  ...
  $rowx = mysql_fetch_array($db_datenx);
  if($rowx["dartefakt"]<getDefaultVariable('sv_max_dartefakt'))
  {
...
//alte nachrichten l�schen
    $tis = time() - (86400 * getDefaultVariable('sv_nachrichten_deldays'));
    $datum = date("YmdHis", $tis);
    mysql_query("DELETE FROM de_user_news where time < $datum", $db);

//alte hyperfunk l�schen
    $tis = time() - (86400 * getDefaultVariable('sv_hf_deldays'));
    $datum = date("YmdHis", $tis);
...
//emaitext
    $betreff = $wt_lang['inaktivenmailbetreff'] . getDefaultVariable('sv_server_tag') . ' - ' . getDefaultVariable('sv_server_name');
    $emailtext = $wt_lang['inaktivenmailbody'];

    $tis = time() - (86400 * (getDefaultVariable('sv_inactiv_deldays') - 1));
...
//inaktive accounts l�schen
    ...
    if ($dodel == 1) {
        ...
        $tis = time() - (86400 * getDefaultVariable('sv_inactiv_deldays'));
        $datum = date("Y-m-d H:i:s", $tis);
        $time = strftime("%Y%m%d%H%M%S");
        ...
        while ($row = mysql_fetch_array($db_daten)) {
            ...
            if ($sector > 0 and $delmode == 0 and $status == 1) {//spieler kommt in den umode und wird nach 0:0 verschoben um danach wieder in Sektor 1 zu landen
                ...
                //mail an den spieler
                $betreff = $wt_lang['loeschmailbetreff'] . getDefaultVariable('sv_server_tag') . ' - ' . getDefaultVariable('sv_server_name');
                ...
                //sou-daten l�schen
                if ($sou_user_id > 0) {
                    mysql_select_db(getDefaultVariable('sv_database_sou'), $db);
                    ...
                    mysql_select_db(getDefaultVariable('sv_database_de'), $db);
                }
            } else {
                @mail_smtp($GLOBALS['env_admin_email'], getDefaultVariable('sv_server_tag') . ' Versuche PA-Spieler zu löschen. user_id: ' . $uid . ' - Spielername: ' . $spielername, ' ');
            }
        }

        //nicht aktivierte account nach 2 tagen l�schen
        $tis = time() - (86400 * getDefaultVariable('sv_not_activated_deldays'));
        $datum = date("Y-m-d H:i:s", $tis);
    ...
    if (getDefaultVariable('sv_ewige_runde') == 1) {
        if ($maxtick >= 1000) {
            ...
            if ($num == 1) {
                ...
                //mail bzgl. EH
                @mail_smtp($GLOBALS['env_admin_email'], getDefaultVariable('sv_server_tag') . ' Ewige Runde: Neuer EH: user_id: ' . $user_id . ' - Spielername: ' . $spielername, ' ');
        ...
        if ($maxtick >= 1000) {
            ...
            if ($num == 1) {
                ...
                //////////////////////////////////////////////////////////////////////
                if ($eh_siege + 1 >= getDefaultVariable('sv_hardcore_need_wins')) {
                    //er ist erhabener
                    ...
                    //if(getDefaultVariable('sv_pcs_id')>0)
                    @mail_smtp($GLOBALS['env_admin_email'], 'Die Runde auf ' . getDefaultVariable('sv_server_tag') . ' ist vorbei.', 'Die Runde auf ' . getDefaultVariable('sv_server_tag') . ' ist vorbei.');
                    ...
                    //mail bzgl. EH
                    @mail_smtp($GLOBALS['env_admin_email'], getDefaultVariable('sv_server_tag') . ' Hardcore: Neuer Teil-EH: user_id: ' . $user_id . ' - Spielername: ' . $spielername, ' ');
        ...
        $row = mysql_fetch_array($db_daten);
        $maxtick = $row["tick"];
        if ($maxtick < 2500000 or getDefaultVariable('sv_comserver_roundtyp') == 1) {
            /*
		...
		$score=$row["score"];*/
            ...
            //getDefaultVariable('sv_winscore') wird in zukunft die rundendauer in ticks angeben
...
//                getDefaultVariable('sv_winscore') += 2500000;//fix f�r community-server in der BR
//            }

            //getDefaultVariable('sv_winscore')=0;
            //////////////////////////////////////////////////////////////////////
            //////////////////////////////////////////////////////////////////////
            //rundenpunkte und creditgewinne beim start des eh-kampfes verteilen
            //////////////////////////////////////////////////////////////////////
            //////////////////////////////////////////////////////////////////////
            if ($score >= getDefaultVariable('sv_winscore') and $roundpointsflag == 0) {
                //info bzgl. start eh-kampf
                @mail_smtp($GLOBALS['env_admin_email'], 'Bei der Runde auf ' . getDefaultVariable('sv_server_tag') . ' hat der Erhabenenkampf begonnen.', 'Bei der Runde auf ' . getDefaultVariable('sv_server_tag') . ' hat der Erhabenenkampf begonnen.');
                ...
                while ($row = mysql_fetch_array($db_daten)) {
                    ...
                    wt_change_credits($row["user_id"], getDefaultVariable('sv_credit_win')[0][$platz], 'Creditgewinn ' . ($platz + 1) . '. Platz Punkte');
                    echo '<br>' . $row["user_id"];
                    $platz++;
                }

                //spieler mit den meisten Executorpunkten
                if (getDefaultVariable('sv_deactivate_trade') == 0) {
                    ...
                    while ($row = mysql_fetch_array($db_daten)) {
                        //credits gutschreiben
                        wt_change_credits($row["user_id"], getDefaultVariable('sv_credit_win')[1][$platz], 'Creditgewinn ' . ($platz + 1) . '. Platz Executorpunkte');
                        echo '<br>' . $row["user_id"];
                        $platz++;
                    }
                }
		  ...
		  {
			//credits gutschreiben
			wt_change_credits($row["user_id"], getDefaultVariable('sv_credit_win')[2][$platz], 'Creditgewinn '.($platz+1).'. Platz Kopfgeldj�ger');
			echo '<br>'.$row["user_id"];
			$platz++;
		  }
		  */
		  ...
		  if(getDefaultVariable('sv_server_lang')==1)
		  {
			...
			{
			  //credits gutschreiben
			  wt_change_credits($row["user_id"], getDefaultVariable('sv_credit_win')[3][$platz], 'Creditgewinn '.($platz+1).'. Platz EFTA-Erfahrungspunkte');
			  echo '<br>'.$row["user_id"];
			  $platz++;
			}
		  }
		  */
            ...
            //////////////////////////////////////////////////////////////////////
            //////////////////////////////////////////////////////////////////////
            if ($score >= getDefaultVariable('sv_winscore')) {
                ...
                if ($winid == $uid) {
                    ...
                    if ($winticks == 1) {
                        ...
                        //dem erhabenen die credits geben
                        echo '<hr>Erhabenercreditgewinn:<br>';
                        wt_change_credits($uid, getDefaultVariable('sv_credit_win')[4][0], 'Creditgewinn Erhabener');
                        echo '<br>' . $uid;
                    ...
                    mysql_query("UPDATE de_system SET winid = $uid, winticks=getDefaultVariable('sv_benticks')", $db);
                    $winticks = getDefaultVariable('sv_benticks');
                    $winid = $uid;
                }
...
*/
        ...
        $ranglistendaten .= "SQL-Logfilelöschung: DELETE FROM gameserverlogdata WHERE serverid=getDefaultVariable('sv_servid') AND time<'$logtime'\n";

        //Ranglistendaten per E-Mail an den Admin schicken
        @mail_smtp($GLOBALS['env_admin_email'], 'Die Runde auf ' . getDefaultVariable('sv_server_tag') . ' ist vorbei - Ranglistendaten', $ranglistendaten);
	...
	";
        ...
        if (getDefaultVariable('sv_auto_reset') == 1) {
