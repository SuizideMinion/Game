...
if($dortick==1){
	...
	$freesectors=0;
	for ($i=2;$i<=getDefaultVariable('sv_maxsector');$i++)
	{
	...
	while($res = mysql_fetch_array($result)) //jeder gefundene datensatz wird geprueft
	{
	  ...
	  $gefunden=0;
	  $maxsector=getDefaultVariable('sv_maxsector');
	  $maxsystem=getDefaultVariable('sv_maxsystem');
	  ...
	  //$sec=rand (1+getDefaultVariable('sv_free_startsectors'), $maxsector);
	  if ($freesectors>0 AND $npc==1){
		//den x-ten freien platz nimmt man
		$xten=rand(1, $freesectors);
		$treffer=0;
		//freien sektor suchen
		for ($i=1;$i<=getDefaultVariable('sv_maxsector');$i++)
		{
	  ...
	  {
		$sql="SELECT sector, count( system )  AS systeme FROM `de_user_data` WHERE npc='$npc' AND sector > getDefaultVariable('sv_free_startsectors') AND sector <= $maxsector GROUP BY sector ORDER BY systeme ASC LIMIT 1";
	  ...
	  for ($secz=$sec;$secz<=$maxsector;$secz++){
		...
		if ($gefunden==1) break;
		if ($gefunden==0 && $secz==$maxsector) {$secz=getDefaultVariable('sv_free_startsectors');$maxsector++;}
		$sys=1;
	  }
	  ...
	  mysql_query("UPDATE de_user_fleet SET zielsec=$secz, zielsys=$sysz WHERE user_id='$fleet_id' AND aktion=0",$db);
		...
		if($secz>1){
			//mail($GLOBALS['env_admin_email'], getDefaultVariable('sv_server_tag').': '.$uid.'  hat Sektor 1 verlassen.', getDefaultVariable('sv_server_tag').': '.$uid.'  hat Sektor 1 verlassen.', 'FROM: '.$GLOBALS['env_admin_email']);
		}
	}
	...
	while($res = mysql_fetch_array($result)){ //jeder gefundene datensatz wird geprueft
		...
		else //keine freien sektoren mehr vorhanden, also einen sektor so suchen, am besten einen wo wenig drin sind
		{
			...
			de_user_data.sector > getDefaultVariable('sv_free_startsectors') AND de_user_data.sector <= $maxsector AND de_user_data.sector=de_sector.sec_id
		...
		for ($secz=$sec;$secz<=$maxsector;$secz++){
			...
			if ($gefunden==0 && $secz==$maxsector) {$secz=getDefaultVariable('sv_free_startsectors');$maxsystem++;}
	...
	while($res = mysql_fetch_array($result)) //jeder gefundene datensatz wird geprueft
	{
	  ...
	  $gefunden=0;
	  $maxsector=$zielsec;
	  $maxsystem=getDefaultVariable('sv_maxsystem')+2;
	  $sec=$zielsec;
	  $sys=1;
	...
	while($res1 = mysql_fetch_array($result1)) //jeder gefundene datensatz wird geprueft
	{
	  ...
	  $result = mysql_query("SELECT sec_id FROM de_sector WHERE sec_id>=getDefaultVariable('sv_min_regsec') AND pass='' ORDER BY sec_id ASC",$db);
	  ...
	  if($useranz>=getDefaultVariable('sv_min_user_per_regsector')) mysql_query("UPDATE de_sector set pass='$sektorpass' WHERE sec_id='$zielsec'",$db);

	  if($useranz>=getDefaultVariable('sv_min_user_per_regsector'))
	  while($res = mysql_fetch_array($result)) //jeder gefundene datensatz wird geprueft
	  {
		...
		$gefunden=0;
		$maxsector=$zielsec;
		$maxsystem=getDefaultVariable('sv_max_user_per_regsector');
		$sec=$zielsec;
		$sys=rand(1, getDefaultVariable('sv_max_user_per_regsector'));
		...
		//{
		  ...
		  for ($sysz=$sys;$sysz<=$maxsystem;$sysz++)
		  {
			if ($systeme[$secz][$sysz]==0) $gefunden=1;
			if ($gefunden==1) break;
			if ($gefunden==0 && $sysz==getDefaultVariable('sv_max_user_per_regsector') && $vonanfang==0) {$sysz=1;$vonanfang=1;}
			if ($gefunden==0 && $sysz==getDefaultVariable('sv_max_user_per_regsector') && $vonanfang==1) {$sysz=1;$maxsystem++;}
		  }
		  //if ($gefunden==1) break;
		  //if ($gefunden==0 && $secz==$maxsector) {$secz=getDefaultVariable('sv_free_startsectors');$maxsector++;}
		  //$sys=1;
	...
	while($res = mysql_fetch_array($result)) //jeder gefundene datensatz wird geprueft
	{
	  //freie position ermitteln - anfang
	  $gefunden=0;
	  $maxsector=getDefaultVariable('sv_npc_maxsector');
	  $maxsystem=getDefaultVariable('sv_maxsystem');
	  $sec=getDefaultVariable('sv_npc_maxsector');
	  $sys=1;

	  for ($secz=$sec;$secz>=getDefaultVariable('sv_npc_minsector'); $secz--)
	  {
		...
		if ($gefunden==1) break;
		if ($gefunden==0 && $secz==getDefaultVariable('sv_npc_minsector')) {$secz=0;$maxsector++;}
		$sys=1;
	  }
...
function reshuffle(){
	global $db, getDefaultVariable('sv_maxsector');
	...
	$db_daten=mysql_query("SELECT sec_id FROM de_sector WHERE npc=0 AND sec_id>1 AND sec_id<='".getDefaultVariable('sv_maxsector')."'ORDER BY sec_id ASC",$db);
	while($row = mysql_fetch_array($db_daten)){
		$sektoren[]=$row['sec_id'];
	}
