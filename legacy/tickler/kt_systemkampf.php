...
for ($c=0; $c<$z; $c++){
  ...
  AND aktion = 1 AND zeit = 1 ORDER BY hsec, hsys";
	...
	while($row = mysql_fetch_array($res)){//f�r jede flotte die daten auslesen
		...
		for($i=0; $i<getDefaultVariable('sv_anz_schiffe'); $i++){
			  $wgrad=1;
  ...
  AND aktion = 2 AND (zeit = 1 OR (zeit = 0 AND aktzeit > 0)) ORDER BY hsec, hsys",$db);
	...
	//schauen ob es eine planetare schildwerweiterung gibt und berechne die st�rke
	 if (hasTech($d_userdata[0]['techs'],30)==1){
		...
		$psext=getDefaultVariable('sv_planetshieldext')[1]+$psextbonus-$psextmalus;
		//spezialisierung
		if($d_userdata[0]['spec3']==1)$psext+=0.1;

		if($psext<getDefaultVariable('sv_planetshieldext')[0])$psext=getDefaultVariable('sv_planetshieldext')[0];
		if($psext>getDefaultVariable('sv_planetshieldext')[2])$psext=getDefaultVariable('sv_planetshieldext')[2];
	...
	//zuerst array nullen
	for ($i=0;$i<getDefaultVariable('sv_anz_rassen');$i++)
	 for ($s=0;$s<getDefaultVariable('sv_anz_schiffe');$s++)
	  $atterrassen[$i][$s]=0;
	...
	for($i=0; $i<(getDefaultVariable('sv_anz_schiffe')+getDefaultVariable('sv_anz_tuerme')); $i++)
	{
	  //hitpoints
	  $atter_hpges[$i]=
	  ...
	  if ($i<getDefaultVariable('sv_anz_schiffe'))
	  $deffer_hpges[$i]=
	  ...
	  $deffer_hpges[$i]=$turm[$i-getDefaultVariable('sv_anz_schiffe')]*$unit[$d_userdata[0][3]-1][$i][1];
	  ...
	  $deffer_awges[$i]=$turm[$i-getDefaultVariable('sv_anz_schiffe')]*$unit[$d_userdata[0][3]-1][$i][2]*$tawartbonus;
	...
	for($i=0; $i<(getDefaultVariable('sv_anz_schiffe')+getDefaultVariable('sv_anz_tuerme')); $i++){
	  ...
	  if($npc==0){
		  ...
		  //echo '<br>BW1:'.$bw.' ';
		  if($bw>0)
		  for($j=0; $j<getDefaultVariable('sv_anz_schiffe')+getDefaultVariable('sv_anz_tuerme'); $j++){
	  ...
	  $bw=$turm[$i-getDefaultVariable('sv_anz_schiffe')]*$unit[$d_userdata[0][3]-1][$i][3]*$tbwartbonus;
	...
	for($i=0; $i<getDefaultVariable('sv_anz_schiffe'); $i++){
		...
		for ($r=0; $r<getDefaultVariable('sv_anz_rassen'); $r++){
			if ($atter_awges[$i]>0)
	...
	for ($i=0;$i<$anz_atter;$i++){
		$rasse=$a_userdata[$i][3]-1;
		for($j=0; $j<getDefaultVariable('sv_anz_schiffe'); $j++){
			if ($atter[$i][$j]>0){
	...
	//deffer - tuerme
	for($i=getDefaultVariable('sv_anz_schiffe'); $i<getDefaultVariable('sv_anz_schiffe')+getDefaultVariable('sv_anz_tuerme'); $i++)
	{
	  //verteilung der geblockten angriffswerte der klasse auf die rasse
	  $awgeblockt=$deffer_awges[$i]-$deffer_awrest[$i];
	  ...
	  if ($unit[$r][$i][2]>0)
	  $deffertuerme_geblockt[$i-getDefaultVariable('sv_anz_schiffe')]=floor($awgeblockt/$unit[$r][$i][2]);
	  else
	  $deffertuerme_geblockt[$i-getDefaultVariable('sv_anz_schiffe')]=0;

	  if($deffertuerme_geblockt[$i-getDefaultVariable('sv_anz_schiffe')]>$turm[$i-getDefaultVariable('sv_anz_schiffe')])$deffertuerme_geblockt[$i-getDefaultVariable('sv_anz_schiffe')]=$turm[$i-getDefaultVariable('sv_anz_schiffe')];

	  //if ($awgeblockt>0) echo '<br>TG:'.$deffertuerme_geblockt[$i-getDefaultVariable('sv_anz_schiffe')].' ';
	}
	//emp-waffen ende
	...
	for($i=getDefaultVariable('sv_anz_schiffe'); $i<getDefaultVariable('sv_anz_schiffe')+getDefaultVariable('sv_anz_tuerme'); $i++){
	  ...
	  if ($unit[$r][$i][1]>0){
		$deffertuerme_zer[$i-getDefaultVariable('sv_anz_schiffe')]=floor($hpzer/$unit[$r][$i][1]);
	  } else{
		$deffertuerme_zer[$i-getDefaultVariable('sv_anz_schiffe')]=0;
	  }
	  ...
	  if ($ps==1)
	  {
		...
		$ps_bonus=getDefaultVariable('sv_ps_bonus')+(getDefaultVariable('sv_ps_bonus')*$defense_bonus_ps/100);
		...
		$deffertuerme_zer[$i-getDefaultVariable('sv_anz_schiffe')]=
		 floor($deffertuerme_zer[$i-getDefaultVariable('sv_anz_schiffe')]-($deffertuerme_zer[$i-getDefaultVariable('sv_anz_schiffe')]/100*$ps_bonus));
	  }

	  //erfahrungspunkte der t�rme der heimatflotte zurechnen - veraltet
	  //$deffer_exp[0]=floor($deffer_exp[0]+($deffertuerme_zer[$i-getDefaultVariable('sv_anz_schiffe')]*$unit[$r][$i][4]/100));
	  //erfahrungspunkte der t�rme direkt bei den t�rmen hinterlegen
	  $defenseexp+=floor($deffertuerme_zer[$i-getDefaultVariable('sv_anz_schiffe')]*$unit[$r][$i][4]/100);
	}
...
*/
	...
	for ($s=0;$s<getDefaultVariable('sv_anz_tuerme');$s++){
		$deffer_pktges+=$turm[$s]*$unit[$rasse][getDefaultVariable('sv_anz_schiffe')+$s][4];
		$deffer_pktges_tuerme+=$turm[$s]*$unit[$rasse][getDefaultVariable('sv_anz_schiffe')+$s][4];
		$deffer_pktges_zer+=$deffertuerme_zer[$s]*$unit[$rasse][getDefaultVariable('sv_anz_schiffe')+$s][4];
		$deffer_pktges_zer_tuerme+=$deffertuerme_zer[$s]*$unit[$rasse][getDefaultVariable('sv_anz_schiffe')+$s][4];
	}
	...
	if($zra>=$zrd){
	  ...
	  $stehlbarekollies=floor($kollies*getDefaultVariable('sv_kollie_klaurate'));
	...
	if ($defenseexp>0 && $sv_oscar!=1){
		...
		//1. feststellen wie viel dem Ziel (nur das Ziel, keine weiteren Deffer an Punkten zerst�rt worden ist
			...
			for ($s=0;$s<getDefaultVariable('sv_anz_tuerme');$s++){
				$target_score_lost+=$deffertuerme_zer[$s]*$unit[$d_userdata[0][3]-1][$s+getDefaultVariable('sv_anz_schiffe')][4];
			}
			...
			for ($i=0;$i<$anz_atter;$i++){
				...
				if(($kg[0]>0 OR $kg[1]>0 OR $kg[2]>0 OR $kg[3]>0) && $target_score_lost>0){
					...
					@mail($GLOBALS['env_admin_email'], getDefaultVariable('sv_server_tag').' KG '.$uid, $text.'<br>'.$nachricht, 'FROM: '.$GLOBALS['env_admin_email']);
					*/
				}
			}
		}
	...
	//echo '<br>ATTER-HP(verloren): '.$vernichtete_atter.' - ATTER-HP(gesamt):'.$atter_allhp.' - ZRA:'.$zra;
	  ...
	  //mysqli_query($dbi_ls, "INSERT INTO ls_de_kb SET time=NOW(), server='getDefaultVariable('sv_server_tag')', atter='$global_atter', deffer='$global_deffer', kb='$global_kb', kbversion=0");
	  //if(gettype($dbi_ls)=='mysqli'){
		  mysqli_query($dbi_ls, "INSERT INTO ls_de_kb SET time=NOW(), server='getDefaultVariable('sv_server_tag')', atter='$global_atter', deffer='$global_deffer', kb='".serialize($global_kb_array)."', kbversion=1");
	  //}
	...
	for ($i=0;$i<$anz_atter;$i++){
		...
		if ($atter_sk[$i]>0){
			...
			$col_angriffsgrenze_final=$col_angriffsgrenze/100*getDefaultVariable('sv_max_col_attgrenze');
			if($col_angriffsgrenze_final>getDefaultVariable('sv_max_col_attgrenze'))$col_angriffsgrenze_final=getDefaultVariable('sv_max_col_attgrenze');
			if($col_angriffsgrenze_final<getDefaultVariable('sv_min_col_attgrenze'))$col_angriffsgrenze_final=getDefaultVariable('sv_min_col_attgrenze');
			//bei npc-zielen ist die kollektorenangriffsgrenze immer der minimumwert
			if($npc==1)$col_angriffsgrenze_final=getDefaultVariable('sv_min_col_attgrenze');
		...
		//kriegsartefakte
		if(getDefaultVariable('sv_kartefakt_exp_atter')>0){
			$kartefakte=floor($atter_exp[$i]/getDefaultVariable('sv_kartefakt_exp_atter'));
		}else{
			$kartefakte=0;
		}
		...
		if($npc==0){
			...
			@mail($GLOBALS['env_admin_email'], getDefaultVariable('sv_server_tag').' KG '.$uid, $text, 'FROM: '.$GLOBALS['env_admin_email']);
			*/
		}
	...
	for ($i=0;$i<$anz_heimatflotten;$i++){
		...
		*/
		...
		if($i==$anz_heimatflotten-1){
			...
			if(getDefaultVariable('sv_kartefakt_exp_deffer')>0)
			$kartefakte=floor(($deffer_kbsum[3][1]+$defenseexp)/getDefaultVariable('sv_kartefakt_exp_deffer'));
			else $kartefakte=0;
	...
	for ($i=$anz_heimatflotten;$i<$anz_deffer;$i++){
		...
		//kriegsartefakte
		if(getDefaultVariable('sv_kartefakt_exp_deffer')>0)
		$kartefakte=floor($deffer_exp[$i]/getDefaultVariable('sv_kartefakt_exp_deffer'));
		else $kartefakte=0;
		$spielerstring=$spielerstring.$kartefakte.';';
		$kb_daten_spieler['kartefakt']=$kartefakte;
	...
	if($rec==1){
		...
		if($sv_oscar!=1){
			...
			$rasse=$d_userdata[0][3]-1;
			if($whg==1)$anteil=getDefaultVariable('sv_recyclotron_bonus_whg'); else $anteil=getDefaultVariable('sv_recyclotron_bonus');
			...
			if($npc==1){
				...
				$rec_secbonus=getDefaultVariable('sv_recyclotron_sector_bonus')/$num*($ownsectorplatz-1);
				//recyclotronbonus darf nicht gr��er als das maximum sein
				if($rec_secbonus>getDefaultVariable('sv_recyclotron_sector_bonus'))$rec_secbonus=getDefaultVariable('sv_recyclotron_sector_bonus');
			...
			if($anteil<getDefaultVariable('sv_recyclotron_min'))$anteil=getDefaultVariable('sv_recyclotron_min');
			if($anteil>getDefaultVariable('sv_recyclotron_max'))$anteil=getDefaultVariable('sv_recyclotron_max');
			...
			//t�rme
			for ($s=getDefaultVariable('sv_anz_schiffe');$s<getDefaultVariable('sv_anz_schiffe')+getDefaultVariable('sv_anz_tuerme');$s++)
			{
			  $restyp01+=($deffertuerme_zer[$s-getDefaultVariable('sv_anz_schiffe')]*$unit[$rasse][$s][5][0]);
			  $restyp02+=($deffertuerme_zer[$s-getDefaultVariable('sv_anz_schiffe')]*$unit[$rasse][$s][5][1]);
			  $restyp03+=($deffertuerme_zer[$s-getDefaultVariable('sv_anz_schiffe')]*$unit[$rasse][$s][5][2]);
			  $restyp04+=($deffertuerme_zer[$s-getDefaultVariable('sv_anz_schiffe')]*$unit[$rasse][$s][5][3]);
			  $restyp05+=($deffertuerme_zer[$s-getDefaultVariable('sv_anz_schiffe')]*$unit[$rasse][$s][5][4]);
			}
