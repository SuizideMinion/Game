...
xecho($varscore);
xecho($varalterplatz);
xecho('for (i=0; i<r.length; i++){var j=i+1;var colag=c[i].replace(".","")/c[0].replace(".","")*'.getDefaultVariable('sv_max_col_attgrenze').';if(colag<'.getDefaultVariable('sv_min_col_attgrenze').')colag='.getDefaultVariable('sv_min_col_attgrenze').';document.write("<tr><td class=\"cc\">"+j+"</td><td class=\"cc\">"+r[i]+"</td><td class=\"cc\">"+s[i]+":"+t[i]+"</td><td class=\"cc\"><a href=\"details.php?se="+s[i]+"&sy="+t[i]+"\">"+n[i]+"</a></td><td class=\"cc\" title=\""+(Math.round(colag*c[i].replace(".","")))+"\">"+c[i]+"</td><td class=\"cr\">"+p[i]+"</td></tr>");}');
xecho('
// -->
</script>
</table>
...
while($row = mysqli_fetch_array($db_daten)){
  ...
  if($row["platz_last_day"]==$row["platz"])
$grafik='<img border="0" src="'.getDefaultVariable('sv_image_server_list')[0].'g/tl_constant.gif" width="11" height="11" title="unver&auml;ndert" alt="unver&auml;ndert">';
  if($row["platz_last_day"]>$row["platz"])
$grafik='<img border="0" src="'.getDefaultVariable('sv_image_server_list')[0].'g/tl_up.gif" width="11" height="11" title="+'.($row['platz_last_day']-$row['platz']).'" alt="+'.($row['platz_last_day']-$row['platz']).'" >';
  if($row["platz_last_day"]<$row["platz"])
$grafik='<img border="0" src="'.getDefaultVariable('sv_image_server_list')[0].'g/tl_down.gif" width="11" height="11" title="-'.($row['platz']-$row['platz_last_day']).'" alt="-'.($row['platz']-$row['platz_last_day']).'">';
