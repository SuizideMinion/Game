...
function issectorcommander() {
	...
	if($sector>0){
		...
		//alle stimmen zählen
		for ($i = 1; $i <= getDefaultVariable('sv_maxsystem'); $i++){
			if(isset($su[$i][1])){
				$ska[$su[$i][1]]++;
			}
		}
		...
		$anzahl=0;
		if ($mw<8)
		{for ($i = 1; $i <= getDefaultVariable('sv_maxsystem'); $i++)if($ska[$i]==$mw)$anzahl++;}
		else $anzahl=1;
...
function getSKSystemBySecID($sec_id){
	...
	//echo '<br>IS_SK: '.$sector.'/'.getDefaultVariable('sv_maxsystem');
