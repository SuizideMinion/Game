...
include "inc/lang/" . getDefaultVariable('sv_server_lang') . "_sinfo.lang.php";
...
<center><br>
    <table width="580" border="0" cellpadding="0" cellspacing="0">
        ...
        <tr>
            <td class="rl">&nbsp;</td>
            <td>
                <div class="cell"><?php echo utf8_encode($detmsg); ?></div>
            </td>
            <td class="rr">&nbsp;</td>
        </tr>
        <tr align="center">
        ...
        <tr>
            <td class="rl">&nbsp;</td>
            <td align="center">
                <div class="cell"><?php echo utf8_encode($kpmsg); ?></div>
            </td>
            <td class="rr">&nbsp;</td>
        </tr>
        <tr>
    ...
    for ($h = 0; $h <= 23; $h++) {
        ...
        for ($m = 0; $m <= 59; $m++) {
            if (in_array(intval($m), getDefaultVariable('wts')[$h])) {
                if (!$first_minute) {
                    echo ',';
                }
                echo $m;
    ...
    echo '<table style="width: 100%; text-align: center;"><tr><td>Stunde</td><td>Minute</td></tr>';
    for ($h = 0; $h <= 23; $h++) {
        if (count(getDefaultVariable('kts')[$h]) > 0) {
            $first_minute = true;
            echo '<tr>';
            echo '<td style="width: 100px;">' . $h . '</td><td>';
            for ($m = 0; $m <= 59; $m++) {
                if (in_array(intval($m), getDefaultVariable('kts')[$h])) {
                    if (!$first_minute) {
                        echo ',';
                    }
                    echo $m;
