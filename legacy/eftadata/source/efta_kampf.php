...
if($kampf==1 OR $kampf==3)
{
  ...
  if ($enm_hpakt<=0)//gegner ist tot
  {
           ...
           */
           ...
           if(getDefaultVariable('sv_efta_in_de')==1)
           {
             ...
             if($killcounter==1)
             {
               if($has_palenium==1)
               {
                 ...
                 if($palenium_anz+$res06>getDefaultVariable('sv_max_palenium'))$res06=getDefaultVariable('sv_max_palenium')-$palenium_anz;
               }
               else $res06=0;
           ...
           if(getDefaultVariable('sv_efta_in_de')==1)
           {
             ...
             if($player_killcounter % 100 ==0 AND $killcounter==1 AND $player_killcounter>0)
             {
               ...
               //@mail($GLOBALS['env_admin_email'],"getDefaultVariable('sv_server_tag') - $ums_spielername - Level: $level - Gegnerlevel: $enm_level", 'FROM: '.$GLOBALS['env_admin_email']);
    ...
    if(getDefaultVariable('sv_efta_in_de')==1 && getDefaultVariable('sv_server_tag')='123')
    {
