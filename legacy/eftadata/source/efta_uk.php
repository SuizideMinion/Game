...
if($_GET["b"]==1)
{
  ...
  if($num>0)
  {
    ...
    if($item_useable==1)
    {
      //schauen ob er die voraussetzungen erf�llt
      if($item_level<=$level)
      {
        //schauen was f�r ein typ es ist
        switch($item_typ){
        case 21: //trank
          //heilen
          //4843
          ...
          if($id==4890)
          {
            //die aktionspunkte erh�hen
            $bewpunkte=$bewpunkte+2500;
            if($bewpunkte>getDefaultVariable('sv_max_efta_bew_punkte'))$bewpunkte=getDefaultVariable('sv_max_efta_bew_punkte');
        ...
        case 25: //weiteres item
            ...
            if($num==1)
  			{
                ...
                //nachricht ausgeben
                $itemmsg.='Du sorgst daf�r, dass ein Weg angelegt wird.';
  			  }else $itemmsg.='Hier ist kein Platz um einen Weg anzulegen.';
  			}else $itemmsg.='Hier ist kein Platz um einen Weg anzulegen.';
          }

          if($id==4870)//stra�enbau
          {
          	//die daten des feldes auslesen auf dem man steht
            $eftadb_daten=mysql_query("SELECT * FROM de_cyborg_map WHERE x='$x' AND y='$y' AND z='$map'",$eftadb);
            $num = mysql_num_rows($eftadb_daten);
            if($num==1)
  			{
                ...
                //nachricht ausgeben
                $itemmsg.='Du sorgst daf�r, dass ein Weg zur Stra&szlig;e ausgebaut wird.';
  			  }else $itemmsg.='Um eine Stra&szlig;e zu bauen mu&szlig; sich an dem Ort bereits ein Weg befinden.';
  			}else $itemmsg.='Um eine Stra&szlig;e zu bauen mu&szlig; sich an dem Ort bereits ein Weg befinden.';
          }
          //dornschlingen�therisierer zusammensetzen
          if($id==4872 OR $id==4873 OR $id==4874)
          {
          	//�berpr�fen ob er alle teile hat
          	$num=0;
          ...
          if($id==4875)//dornschliengen�therisierer
          {
			...
			$y1=$y-2;
          	...
          	//erfahrungspunkte f�r den einsatz
          	$expgew=$num*5;
          	give_exp($expgew);
          ...
          if($id==4876 OR $id==4877 OR $id==4878 OR $id==4879 OR $id==4880 OR $id==4881 OR $id==4882 OR $id==4883 OR $id==4884 OR $id==4885 OR $id==4886 OR $id==4887 OR $id==4888 OR $id==4889)
          {
          	...
          	//�berpr�fen ob er gr��er ist als der alte
			if($newbackpacksize>$maxbackpack)
			{
			  //rucksackgr��e updaten
...
if($_GET["b"]==2)
{
  ...
  {
  	case 1:
  	  //�berpr�fen ob es schon m�glich ist
  	  if($player_cooldown1<time())
  	  {
  	  //�berpr�fen ob genug credits vorhanden sind
...
//zuerst items laden, was man tr�gt
