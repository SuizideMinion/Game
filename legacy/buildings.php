...
include 'inc/lang/'.getDefaultVariable('sv_server_lang').'_buildings.lang.php';
...
if ($t>0 && $buildgnr==0 && $_SESSION['build_sec_token']==$_REQUEST['st']){ //ja, es wurde ein button gedrueckt
	...
	if (setLock($ums_user_id)){
		...
		/*
		if(getDefaultVariable('sv_deactivate_efta')==0)
		{
		  if ($t==28)
		  {
...
for ($i=0; $i<$num; $i++) //jeder gefundene datensatz wird geprueft
{
  ...
  if(mysql_result($db_daten, $i, "tech_id")==26 AND getDefaultVariable('sv_max_secmoves')==0){$z1=1;$z2=2;}
