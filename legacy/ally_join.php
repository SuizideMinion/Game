...
include('inc/lang/' . getDefaultVariable('sv_server_lang') . '_ally.join.lang.php');
...
//            $clan = str_replace("'", "\'", $clan);
            ...
            if ($nb > 0) {
                ...
                $antrag = htmlentities($_POST['antrag'], ENT_QUOTES);
                $antrag = str_replace("\n", "<br>", $antrag);
    ...
    $antrag_allyname = $antrag_antrag = null;
    ...
    $result = $db->query($query);
    $nb = $result ? $result->num_rows : 0;

    $active_a_request = empty($antrag_allyname)
        ? $allyjoin_lang['msg_12']
        : $allyjoin_lang['msg_13_1'] . ' <strong>' . htmlentities($antrag_allyname) . '</strong> ' . $allyjoin_lang['msg_13_2'] . ':<br /><br />' . htmlentities($antrag_antrag);

    print('<table width="600" class="cell">
    <tr>
        <td><h3>' . htmlentities($allyjoin_lang['aktivebewerbung']) . ': </h3></td>
    </tr>
    <tr>
        <td>' . $active_a_request . '</td>
    </tr>
    <tr>
        <td><hr></td>
    </tr>
    ...
    print('<table width="600" class="cell">
    <tr>
        <td><h3>' . htmlentities($allyjoin_lang['neuebewerbung']) . ': </h3></td>
    </tr>
    <tr>
        <td>
            <form name="register" method="POST" action="ally_join.php">
                <input type="hidden" name="ally_id" value="' . intval($ally_id) . '">
                ' . htmlentities($allyjoin_lang['msg_14_1']) . ' <strong>' . htmlentities($a_name) . '</strong> ' . htmlentities($allyjoin_lang['msg_14_2']) . ':<br>
                <textarea name="antrag" cols="70" rows="7" wrap="virtual"></textarea>
                <br><br><input type="submit" value="' . htmlentities($allyjoin_lang['bewerbungsenden']) . '" name="ok">
            </form>
        </td>
    </tr>
</table>');
