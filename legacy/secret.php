...
if (!hasTech($pt, 9)) {
    ...
    if ((isset($_GET["a"]) && $_GET["a"] == 'd') || $showscanhistory == 1) {
        ...
        if ($num == 1)//die koordinaten stimmen, gib die daten aus
        {
            ...
            //allianz
            $allianz = '?';
            if ($row["allytag"] != '') $allianz = $row["allytag"];
            if ($row["atime"] > 0) $allianz .= ' (' . date("d.m.Y - G:i", $row["atime"]) . ')';
            //schiffs�bersicht
            $ftime = '?';
            if ($row["ftime"] > 0) $ftime = date("d.m.Y - G:i", $row["ftime"]);
            $e81 = '?';
            if ($row["ftime"] > 0) $e81 = number_format($row["e81"], 0, "", ".");
            $e82 = '?';
            if ($row["ftime"] > 0) $e82 = number_format($row["e82"], 0, "", ".");
            $e83 = '?';
            if ($row["ftime"] > 0) $e83 = number_format($row["e83"], 0, "", ".");
            $e84 = '?';
            if ($row["ftime"] > 0) $e84 = number_format($row["e84"], 0, "", ".");
            $e85 = '?';
            if ($row["ftime"] > 0) $e85 = number_format($row["e85"], 0, "", ".");
            $e86 = '?';
            if ($row["ftime"] > 0) $e86 = number_format($row["e86"], 0, "", ".");
            $e87 = '?';
            if ($row["ftime"] > 0) $e87 = number_format($row["e87"], 0, "", ".");
            $e88 = '?';
            if ($row["ftime"] > 0) $e88 = number_format($row["e88"], 0, "", ".");
            $e89 = '?';
            if ($row["ftime"] > 0) $e89 = number_format($row["e89"], 0, "", ".");
            $e90 = '?';
            if ($row["ftime"] > 0) $e90 = number_format($row["e90"], 0, "", ".");

            //turmübersicht
            $dtime = '?';
            if ($row["dtime"] > 0) $dtime = date("d.m.Y - G:i", $row["dtime"]);
            $e100 = '?';
            if ($row["dtime"] > 0) $e100 = number_format($row["e100"], 0, "", ".");
            $e101 = '?';
            if ($row["dtime"] > 0) $e101 = number_format($row["e101"], 0, "", ".");
            $e102 = '?';
            if ($row["dtime"] > 0) $e102 = number_format($row["e102"], 0, "", ".");
            $e103 = '?';
            if ($row["dtime"] > 0) $e103 = number_format($row["e103"], 0, "", ".");
            $e104 = '?';
            if ($row["dtime"] > 0) $e104 = number_format($row["e104"], 0, "", ".");

            //sondenbericht
            $stime = '?';
            if ($row["stime"] > 0) $stime = date("d.m.Y - G:i", $row["stime"]);
            $score = '?';
            if ($row["stime"] > 0) $score = number_format($row["score"], 0, "", ".");
            $fleet = '?';
            if ($row["stime"] > 0) $fleet = number_format($row["fleet"], 0, "", ".");
            $defense = '?';
            if ($row["stime"] > 0) $defense = number_format($row["defense"], 0, "", ".");
            $build = '?';
            if ($row["stime"] > 0) $build = number_format($row["build"], 0, "", ".");
            $col = '?';
            if ($row["stime"] > 0) $col = number_format($row["col"], 0, "", ".");
            $buildings = '?';
            if ($row["stime"] > 0) $buildings = number_format($row["buildings"], 0, "", ".");
            $restyp01 = '?';
            if ($row["stime"] > 0) $restyp01 = number_format($row["restyp01"], 0, "", ".");
            $restyp02 = '?';
            if ($row["stime"] > 0) $restyp02 = number_format($row["restyp02"], 0, "", ".");
            $restyp03 = '?';
            if ($row["stime"] > 0) $restyp03 = number_format($row["restyp03"], 0, "", ".");
            $restyp04 = '?';
            if ($row["stime"] > 0) $restyp04 = number_format($row["restyp04"], 0, "", ".");
            $restyp05 = '?';
            if ($row["stime"] > 0) $restyp05 = number_format($row["restyp05"], 0, "", ".");
                            ...
                            while ($row = mysql_fetch_array($db_daten)) //jeder gefundene datensatz wird ausgegeben
                            {
                                ...
                                switch ($n) {
                                    case 8:
                                        ...
                                        $tronic = $werte[0];
                                        unset($na);
                                        include "inc/lang/" . getDefaultVariable('sv_server_lang') . "_wt_tronicmsg.lang.php";
