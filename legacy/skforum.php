...
include 'inc/lang/'.getDefaultVariable('sv_server_lang').'_skforum.lang.php';
...
<script src="<?=getDefaultVariable('sv_server_lang')?>_jssammlung.js" type="text/javascript"></script>
...
if($newthread)
{
	...
	elseif(!$error)
  {
    ...
    $i=1;
    $seen="s";
    while($i<getDefaultVariable('sv_maxsector')+1)
    {
    if($i==$sector)
    $seen=$seen.'1';
    else
...
if ($a=="c")
{
  //schaue ob die threadid und der sektor g�ltig sind
  $db_daten=mysql_query("SELECT sector FROM de_sectorforum_threads WHERE id='$threadid'");
  $num = mysql_num_rows($db_daten);
  if ($num==0)$error.=$skforum_lang[msg_5];
  $row = mysql_fetch_array($db_daten);
  if ("0"!=$row[sector] && in_array($ums_user_id, getDefaultVariable('mods')))$error.=$skforum_lang[msg_6];
