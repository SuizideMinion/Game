...
function getAveragePlayerAmountInSectorOnServer()
{
    ...
    $avg = $row['anzahl'] / (getDefaultVariable('sv_maxsector') - 2);
...
function createAuction($uid, $sv_deactivate_vsystems, $ua_name, $db)
{
//    global getDefaultVariable('sv_deactivate_vsystems'), $ua_name, $db;
    ...
    //neue Rohstoffe 3-12
    if (getDefaultVariable('sv_deactivate_vsystems') != 1 && $maxtick > 3000) {
        for ($r = 3; $r <= 12; $r++) {
...
function showeinheit_ang($techname, $tech_id, $rt01, $rt02, $rt03, $rt04, $rt05, $tech_ticks, $vorhanden, $bg, $tooltipid, $has_tech, $item_kosten = '')
{
    ...
    $tooltip = '<img src="' . $ums_gpfad . 'g/' . $ums_rasse . '_hilfe.gif" border="0" title="">';
...
function get_player_allyid($user_id, $db = 0)
{
    if ($db == 0) global $db;
...
function insert_chat_msg($channel, $channeltyp, $spielername, $chat_message)
{
    ...
    if ($channeltyp == 3) {//gloabler Chat
		...
		('$channel', '$channeltyp', 'getDefaultVariable('sv_server_tag')', '$spielername', '$chat_message', '$time', '$owner_id')", $soudb);
    } else {
    ...
    if ($channeltyp == 3) {
        ...
        $message = getDefaultVariable('sv_server_tag') . ' ' . $spielername . ': ' . $chat_message;
    }

    //Allgemeiner Serverchat
    if ($channeltyp == 2) {
        $webhook[] = $webhooks[getDefaultVariable('sv_server_tag')];
...
function showeinheit2($techname, $tech_id, $rt01, $rt02, $rt03, $rt04, $rt05, $tech_ticks, $vorhanden, $bg, $tooltipid = 0)
{
    global $ums_gpfad, $ums_rasse, $sv_link, $tooltips;
    if(!isset($sv_link)) include 'inc/1_links.inc.php';
//    $tooltip = '<img src="' . $ums_gpfad . 'g/t/' . $ums_rasse . '_' . $tech_id . '.jpg" border="0" title="' . $tooltips[$tooltipid] . '">';
    $tooltip = '<img src="' . $ums_gpfad . 'g/t/' . $ums_rasse . '_' . $tech_id . '.jpg" border="0" title="">';
...
{
    global $db, $sv_schiffsdaten, $sv_bs_speedup;
    if(!isset($sv_schiffsdaten)) include 'inc/schiffsdaten.inc.php';
    ...
    $bs_speedup = 0;
    if ($ez[2] >= $ez[4] * $sv_bs_speedup[$rasse - 1][0]
        and
        $ez[3] >= $ez[4] * $sv_bs_speedup[$rasse - 1][1]
    )
        $bs_speedup = 1;
	...
	$anzs=getDefaultVariable('sv_anz_schiffe');
	//reisezeiten laden
	for($x=0;$x<getDefaultVariable('sv_anz_schiffe');$x++)
	{
