...
if(isset($_SESSION['ums_user_id']) && $_SESSION['ums_user_id']>0){
	...
	if(getDefaultVariable('sv_log_player_actions') == 1){
	...
	//aktivitätsbonus
	if(time()>$ic_activetime+getDefaultVariable('sv_activetime') AND $eftachatbotdefensedisable!=1 AND $givenocredit!=1){
		mysql_query("UPDATE de_login SET activetime=".time()." WHERE user_id='$ums_user_id'  AND status=1",$db);
		if($user_credits<getDefaultVariable('sv_credits_max_collect'))
		{
