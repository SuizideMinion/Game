...
<?php
	...
	if (isset($_GET["aenable"])) {
		...
		$NText = 'Ihre Zeitungsausgabe mit dem Titel "'.$AData["titel"].'" wurde von der Redaktion der DigiPaper gepr�ft und f�r gut befunden. Die Ausgabe wird daher nun von DigiPaper vertrieben und steht der gesamten Galaxie zur Verf�gung. Herzlichen Gl�ckwunsch und Danke f�r die Verwendung von DigiPaper.';
		...
		if (trim($ZData["abonenten"]) != "") {
			...
			$NText = 'Es ist eine neue Ausgabe der Zeitung "'.$ZData["name"].'" mit dem Titel "'.$AData["titel"].'" in DigiPaper verf�gbar. Sie bekommen diese Nachricht, da Sie die Zeitung abonniert haben. Sollten Sie kein Interesse mehr daran haben, k�nnen Sie das Abonnement jederzeit in einer unserer Fillialen k�ndigen.';
			if (count($UAbo) > 0) {
	...
	if (isset($_GET["ausgaben"])) {
		echo '<span style="font-size: 10pt; font-weight: bold;"><a href="'.$PHP_SELF.'?site">[ Zur�ck zur �bersicht ]</a></span><br><br>';
		...
		while ($AData = mysql_fetch_assoc($DBData)) {
			...
			echo '<td>[<a href="'.$PHP_SELF.'?zid='.$_GET["zid"].'&aid='.$AData["id"].'&adelete">L�schen</a>]</td>';
	...
	if (isset($_GET["amove"])) {
		echo '<span style="font-size: 10pt; font-weight: bold;"><a href="'.$PHP_SELF.'?zid='.$_GET["zid"].'&ausgaben&uid='.$_GET["uid"].'">[ Zur�ck zur Ausgaben-�bersicht ]</a></span><br><br>';
	...
	if (isset($_GET["read"])) {
		...
		echo '<span style="font-size: 10pt; font-weight: bold;"><a href="'.$PHP_SELF.'?zid='.$AData["zid"].'&ausgaben&uid='.$_GET["uid"].'">[ Zur�ck zur Ausgaben-�bersicht ]</a></span><br><br>';
	...
	if (isset($_GET["delete"])) {
		echo '<span style="font-size: 10pt; font-weight: bold;"><a href="'.$PHP_SELF.'?site">[ Zur�ck zur �bersicht ]</a></span><br><br>';
		echo '<span style="font-size: 10pt; font-weight: bold; color: #FF0000;">Zeitung mit der ID '.$_GET["zid"].' wirklich komplett l�schen?</span><br><br>';
		echo '<form method="post" action="'.$PHP_SELF.'?ok">';
		echo ' <input type="hidden" name="zid" value="'.$_GET["zid"].'">';
		echo ' <input type="submit" name="delzeitung" value="Ja, Zeitung l�schen">';
		echo '</form>';
		echo '</center></body></html>';
		die;
	}
	...
	if (isset($_GET["change"])) {
	  ...
	  echo ' <input type="button" value="URL" onclick="namedlink(document.a_form,\'URL\')">';
		...
		echo '<br><br>';
		echo '<input type="submit" name="savechange" value="�nderungen Speichern">';
		echo '</form><br><br><br>';
	...
	if (isset($_GET["adelete"])) {
		echo '<span style="font-size: 10pt; font-weight: bold;"><a href="'.$PHP_SELF.'?site">[ Zur�ck zur �bersicht ]</a></span><br><br>';
		echo '<span style="font-size: 10pt; font-weight: bold; color: #FF0000;">Ausgabe mit der ID '.$_GET["aid"].' wirklich l�schen?</span><br><br>';
		echo '<form method="post" action="'.$PHP_SELF.'?ok">';
		echo ' <input type="hidden" name="zid" value="'.$_GET["zid"].'">';
		echo ' <input type="hidden" name="aid" value="'.$_GET["aid"].'">';
		echo ' <input type="submit" name="delausgabe" value="Ja, Ausgabe l�schen">';
		echo '</form>';
		echo '</center></body></html>';
		die;
	}
    ...
    foreach ($_POST as $delname => $delwert) {
      ...
      if (is_numeric($delid)) {
		...
		echo 'Es wurden '.$DCount.' Zeitungen inkl. deren Ausgaben gel�scht!<br><br><br>';
	}
?>
...
<?php
	...
	while ($ZData = mysql_fetch_assoc($DBData)) {
		...
		echo '  <td><nobr>[<a href="'.$PHP_SELF.'?zid='.$ZData["id"].'&delete">L�schen</a>]</nobr></td>';
...
<br>
<input type="submit" name="delmarked" value="Markierte Zeitungen l�schen">

</form>

<br><br><br>
&copy;2005 DJ16EL, Ren� Sch�dlich
