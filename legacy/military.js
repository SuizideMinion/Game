                ...
                for (var x = 0; x <= anzs; ++x) {
                    if (aktf[x][rz] != 0) {
                        if ((x != tragbars[0]) && (x != tragbars[1])) {
                            ...
                            if (reisez[x][0] > z1) {
                                z1 = reisez[x][0] - bsspeedupmod;
                            }
                            if (reisez[x][1] > z2) {
                                z2 = reisez[x][1] - bsspeedupmod;
                            }
                            if (reisez[x][2] > z3) {
                                z3 = reisez[x][2] - bsspeedupmod;
                            }
                        }
                    }
                }
...
function DoFleetAction(iFleet, sVar) {
    ...
    switch (iMode) {
        ...
        case "4":
            for (var x = 0; x <= anzs; ++x) {
                ...
                if (iVal != -1) {
                    document.getElementById("m" + (x + 1) + "_" + iVal).value = addPkt(aktf[x][(iVal - 1)]);
                }
    ...
    if (iFleet != 0) {
        document.getElementById("fs_" + iFleet).options.selectedIndex = 0;
    }
    GetSetTrager();
}
