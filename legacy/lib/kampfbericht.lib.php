...
function showkampfberichtV0($text, $rasse, $ums_spielername, $sector, $system, $schiffspunkte)
{
    ...
    unset($kbl_lang);
    include basePath('inc/lang/' . getDefaultVariable('sv_server_lang') . '_kampfbericht.lib.lang.php');
  ...
  <td class="k2">' . $kbl_lang['ueberlebt'] . '</td>
    ...
    ////////////////////////////////////////////////
    $grundindex1 = 0;
    for ($aktrasse = 0; $aktrasse < getDefaultVariable('sv_anz_rassen'); $aktrasse++) {
        if ($krassenvorhanden[$aktrasse] > 0) {
            ...
            for ($i = 0; $i < getDefaultVariable('sv_anz_schiffe'); $i++) {
                if ($schiffsnamen[$i] != 'NA') {
                    if ($c1 == 0) {
                        $c1 = 1;
                        $klasse = $rassenklassen[$aktrasse][0];
                    } else {
                        $c1 = 0;
                        $klasse = $rassenklassen[$aktrasse][1];
                    }
  ...
  ';
        ...
        $c1 = 0;
        $c2 = 0;
        for ($i = 0; $i < getDefaultVariable('sv_anz_tuerme'); $i++) {
            if ($schiffsnamen[$i] != 'NA') {
                if ($c1 == 0) {
                    $c1 = 1;
                    $klasse = $rassenklassen[$aktrasse][0];
...
function showkampfberichtV1($text, $rasse, $ums_spielername, $sector, $system, $schiffspunkte)
{
    ...
    //sprachdatei einbinden
    unset($kbl_lang);
    include basePath('inc/lang/' . (getDefaultVariable('sv_server_lang') ?? 1) . '_kampfbericht.lib.lang.php');
  ...
  <td class="k2">' . $kbl_lang['ueberlebt'] . '</td>
    ...
    if ($geseinheiten_atter_anz !== 0) {
        ...
        $prozent_atter = 0; // Oder ein anderer sinnvoller Wert, der den Fall behandelt, wenn keine Einheiten vorhanden sind
    }
    if ($geseinheiten_deffer_anz > 0) $prozent_deffer = $geseinheiten_deffer_anz_verloren * 100 / $geseinheiten_deffer_anz;
    else $prozent_deffer = 0;
    $kbstring .= '
    ...
    if ($geseinheiten_atter_score !== 0) {
        ...
        $prozent_atter = 0; // Oder ein geeigneter Default-Wert oder eine spezielle Behandlung des Falls
    }
    if ($geseinheiten_deffer_score > 0) $prozent_deffer = $geseinheiten_deffer_score_lost * 100 / $geseinheiten_deffer_score;
    else $prozent_deffer = 0;
    $kbstring .= '
