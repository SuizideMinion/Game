...
class map_system{
					...
					if(!$destroyed && $this->playerBldg[$bldg_index]['bldg_level'] < count($GLOBALS['map_buildings'][$this->playerBldg[$bldg_index]['bldg_id']]['bldg_cost'])){
						...
						if($baukosten['has_all'] && $_POST['upgrade']==1 && $mainbuilding_ok){
							...
							if(time()<$this->playerBldg[$bldg_index]['bldg_time']){
								...
								$upgrade_time=$this->playerBldg[$bldg_index]['bldg_time']+(round($GLOBALS['map_buildings'][$this->playerBldg[$bldg_index]['bldg_id']]['bldg_time']*getDefaultVariable('tech_build_time_faktor')*$GLOBALS['duration_factor']*($this->playerBldg[$bldg_index]['bldg_level']+1)));
								//die('AAAAAAAAA');
							}else{
								//es läuft kein Upgrade
								$upgrade_time=time()+(round($GLOBALS['map_buildings'][$this->playerBldg[$bldg_index]['bldg_id']]['bldg_time']*getDefaultVariable('tech_build_time_faktor')*$GLOBALS['duration_factor']*($this->playerBldg[$bldg_index]['bldg_level']+1)));
								//die('BBBBBBBBB');
							}
							...
							//$content.='<br><br>Verbleibende Bauzeit: <span id="build_counter"></span>';
							//$content.='<script>ang_countdown('.(round($GLOBALS['map_buildings'][$this->playerBldg[$bldg_index]['bldg_id']]['bldg_time']*getDefaultVariable('tech_build_time_faktor')*($this->playerBldg[$bldg_index]['bldg_level']+1))).',"build_counter",0)</script>';
							$content.='
							<script>
							location.href=\'map_system.php?id='.$this->system_id.'\';
							</script>';
							...
							/*
							ang_countdown('.(round($GLOBALS['map_buildings'][$this->playerBldg[$bldg_index]['bldg_id']]['bldg_time']*getDefaultVariable('tech_build_time_faktor')*($this->playerBldg[$bldg_index]['bldg_level']+1))).',"build_counter'.$fieldid.'",0);
							...
							*/
							...
							//Bauzeit ausgeben
							$content.='<div><br>Bauzeit: '.round($GLOBALS['map_buildings'][$this->playerBldg[$bldg_index]['bldg_id']]['bldg_time']*getDefaultVariable('tech_build_time_faktor')*$GLOBALS['duration_factor']*($this->playerBldg[$bldg_index]['bldg_level']+1)).' Sekunden</div>';
							...
							&& in_array($this->fields[$fieldid][0], $GLOBALS['map_buildings'][$g]['need_field_typ'])
								...
								if(!isset($_POST['build']) || $_POST['build']==$g){
									...
									if($baukosten['has_all'] && $has_tech && $_POST['build']==$g){
										...
										setBldgByFieldID($_SESSION['ums_user_id'], $this->system_id, $fieldid, $g, 1, time()+($GLOBALS['map_buildings'][$g]['bldg_time']*getDefaultVariable('tech_build_time_faktor')*$GLOBALS['duration_factor']));
										...
										//Info an Spieler mit laufendem Bau
										//$content.='<br><br>Verbleibende Bauzeit: <span id="build_counter"></span>';
										//$content.='<script type="text/javascript">ang_countdown('.($GLOBALS['map_buildings'][$g]['bldg_time']*getDefaultVariable('tech_build_time_faktor')).',"build_counter",0)</script>';
										$content.='
										<script>
										location.href=\'map_system.php?id='.$this->system_id.'\';
										</script>';
										...
										//Bauzeit ausgeben
										$content.='<div><br>Bauzeit: '.round($GLOBALS['map_buildings'][$g]['bldg_time']*getDefaultVariable('tech_build_time_faktor')*$GLOBALS['duration_factor']).' Sekunden</div>';
	...
	public function buildOutpost(){
		...
		if(hasTech($GLOBALS['pt'],$tech_id)){
			...
			//dauer
			$content.='<br><br>Flotten-Missionsdauer: '.round($GLOBALS['map_buildings'][$bldg_id]['bldg_time']*getDefaultVariable('tech_build_time_faktor')*$GLOBALS['duration_factor']).' Sekunden';
						...
						if($fleet_data[$f]['aktion']==0){
							...
							if($fleet_fk[$f]>=$GLOBALS['map_buildings'][$bldg_id]['bldg_need_fk']){
								if(isset($_REQUEST['action']) && $_REQUEST['action']=='createoutpost' && isset($_REQUEST['fleet_id']) && $_REQUEST['fleet_id']==$f){
									...
									//Gebäude hinterlegen
									setBldgByFieldID($_SESSION['ums_user_id'], $this->system_id, 0, $bldg_id, 1, time()+round($GLOBALS['map_buildings'][$bldg_id]['bldg_time']*getDefaultVariable('tech_build_time_faktor')*$GLOBALS['duration_factor']));

									//Flotte updaten
									$time=round(time()+$GLOBALS['map_buildings'][$bldg_id]['bldg_time']*getDefaultVariable('tech_build_time_faktor')*$GLOBALS['duration_factor']);
									unset($mission_data);
