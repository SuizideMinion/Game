...
include 'inc/lang/'.getDefaultVariable('sv_server_lang').'_techtree.lang.php';
...
else
{
  ...
  if(getDefaultVariable('sv_max_secmoves')==0)$db_daten=mysql_query("SELECT * FROM de_tech_data$ums_rasse WHERE tech_id<120 AND tech_id<>26 ORDER BY tech_id",$db);
  ...
  while($row = mysql_fetch_array($db_daten))
  {
    ...
    //den gr��ten tech_id wert speichern
  ...
  for($i=1;$i<=$maxid;$i++)
  {
    ...
    if($data[$i][0]!='')
    {
      ...
      //echo "Vorbedingung erf�llt";
      //wenn alle voraussetzungen erf�llt sind, dann gr�n, ansonsten rot
      if ($z1==$z2){$bg='ccg';$data[$i][11]=1;$test=1;}else{$bg='ccr';$data[$i][11]=3;$test=3;}
      //bei geb�uden, forschungen,die "gr�n" sind, noch schauen, ob sie schon gebaut sind, falls nicht, gelb anzeigen
