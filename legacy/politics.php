...
include 'inc/lang/'.getDefaultVariable('sv_server_lang').'_politics.lang.php';
...
if(!empty($getnewsec) && $secmoves<getDefaultVariable('sv_max_secmoves') && $techs[26]=='1'){
...
if(!empty($joinsec) && $secmoves<getDefaultVariable('sv_max_secmoves') && $secpass!='' && $techs[26]=='1'){
  ...
  if ($anz2>0 AND $anz3==0) //der sektor besteht schon, direkt reinmoven typ 2
  {
    ...
    if($accanz<getDefaultVariable('sv_max_user_per_regsector'))
    {
  ...
  {
    //schauen ob noch platz im sektor ist
    if($anz1<getDefaultVariable('sv_max_user_per_regsector'))
    {
      //es ist noch ein platz frei -> spieler zieht um
      //eintrag in der db machen, dass er umzieht
...
if (!empty($setvoteout) && ($vspielerwahl==$politics_lang["ja"] || $vspielerwahl==$politics_lang["nein"] || $vspielerwahl==$politics_lang["egal"])){
  ...
  if ($anz1!=0){
    ...
    $jastimmen=0;
    for ($i=1; $i<=(getDefaultVariable('sv_maxsystem')+5); $i++)if ($vvotes[$i]=='1')$jastimmen++;
    ...
    if ($prozentwert>=getDefaultVariable('sv_voteoutgrenze')){
      ...
      if($accstatus==2 OR $accstatus==3){
		...
		//$votetimer=0;
		getDefaultVariable('sv_sector_votetime_lock')=0;
		//if($accstatus!=2)
		mysql_query("UPDATE de_sector SET votetimer='$votetimer', votecounter='getDefaultVariable('sv_sector_votetime_lock')' WHERE sec_id='$sector'",$db);
  ...
  if ($gesperrt==1)
  {
    if($votecounter==0)
    {
  	  if ($anz1==0 AND $anz2>0)
      {
        ...
        $vvotes='s';
        for ($i=1; $i<=(getDefaultVariable('sv_maxsystem')+5); $i++) $vvotes.='3';
...
if ($s==2){
	...
	*/
	...
	echo '<input type="text" name="newbk" value="'.$bk.'" size="4" maxlength="2"> ('.$politics_lang["system"].' 1-'.getDefaultVariable('sv_maxsystem').')<br><br>';
	?>
	</div>
	</td>
	<td width="13" class="rr">&nbsp;</td>
...
for ($i = 1; $i <= (getDefaultVariable('sv_maxsystem')+5); $i++){
...
$c1=0;
for ($j = 1; $j <= (getDefaultVariable('sv_maxsystem')+5); $j++){
...
if ($techs[26]=='1'  AND $secmoves<getDefaultVariable('sv_max_secmoves'))
{
?>
<tr align="center">
<td width="13" height="37" class="rml">&nbsp;</td>
...
else
{
  ...
  echo '<br>'.$politics_lang["msg_20_1"].' '.$useranz.' '.$politics_lang["msg_20_2"].' '.getDefaultVariable('sv_min_user_per_regsector').' '.$politics_lang["msg_20_3"].'.<br>';
...
echo $politics_lang["msg_23_1"].' '.number_format($prozentwert, 2,",",".").'% '.$politics_lang["msg_23_2"].' '.getDefaultVariable('sv_voteoutgrenze').'%, '.$politics_lang["msg_23_3"].'.<br>';
