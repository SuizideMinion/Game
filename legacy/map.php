...
include 'inc/lang/'.getDefaultVariable('sv_server_lang').'_sector.lang.php';
...
echo '<div id="serverdesc" style="top:40512px; left:40000px;" data-left="40000" data-top="40512">'.getDefaultVariable('sv_server_name').' '.getDefaultVariable('sv_server_tag').'</div>';
...
$sec_angriffsgrenze=getDefaultVariable('sv_attgrenze')-getDefaultVariable('sv_attgrenze_whg_bonus');
...
for($player_sector=0;$player_sector<$sec_anzahl;$player_sector++){
	...
	*/
	...
	if($sec_data['npc']==1){
		...
		$sec_angriffsgrenze=getDefaultVariable('sv_attgrenze')-getDefaultVariable('sv_attgrenze_whg_bonus')+$sec_malus;
...
function wellenrechner($kol, $maxcol, $npcsec){
	global $sec_lang, $col, getDefaultVariable('sv_min_col_attgrenze'), getDefaultVariable('sv_max_col_attgrenze'), $ums_premium, getDefaultVariable('sv_kollie_klaurate');
	...
	if($ums_premium==1){
		...
		for($we=0; $we<5; $we++){
			...
			if($npcsec==0){
	  			...
	  			if($col_angriffsgrenze>getDefaultVariable('sv_max_col_attgrenze'))$col_angriffsgrenze=getDefaultVariable('sv_max_col_attgrenze');
	  			if($col_angriffsgrenze<getDefaultVariable('sv_min_col_attgrenze'))$col_angriffsgrenze=getDefaultVariable('sv_min_col_attgrenze');
	  		}else{
