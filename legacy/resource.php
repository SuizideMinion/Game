...
include 'inc/lang/' . getDefaultVariable('sv_server_lang') . '_resource.lang.php';
...
//werberlink erzeugen //TODO:: ? wird nicht mehr gebraucht ?
...
if (($maxtick > 2500000 && getDefaultVariable('sv_ewige_runde') != 1 && getDefaultVariable('sv_hardcore') != 1) or (getDefaultVariable('sv_comserver') == 1 and getDefaultVariable('sv_comserver_roundtyp') == 1)) {
...
if (!hasTech($pt, 4)) {//keine gilde
    $grundm = getDefaultVariable('sv_plan_grundertrag')[0] * $grundertragmultiplikator * (1 + $planertragbonus);
    $grundd = getDefaultVariable('sv_plan_grundertrag')[1] * $grundertragmultiplikator * (1 + $planertragbonus);
    $grundi = getDefaultVariable('sv_plan_grundertrag')[2] * $grundertragmultiplikator * (1 + $planertragbonus);
    $grunde = getDefaultVariable('sv_plan_grundertrag')[3] * $grundertragmultiplikator * (1 + $planertragbonus);

    //bonus durch spezialisierung
    $spezim = getDefaultVariable('sv_plan_grundertrag')[0] * $grundertragmultiplikator * $planertragbonus;
    $spezid = getDefaultVariable('sv_plan_grundertrag')[1] * $grundertragmultiplikator * $planertragbonus;
    $spezii = getDefaultVariable('sv_plan_grundertrag')[2] * $grundertragmultiplikator * $planertragbonus;
    $spezie = getDefaultVariable('sv_plan_grundertrag')[3] * $grundertragmultiplikator * $planertragbonus;
} else {  //mit gilde
    $grundm = getDefaultVariable('sv_plan_grundertrag_whg')[0] * $grundertragmultiplikator * (1 + $planertragbonus);
    $grundd = getDefaultVariable('sv_plan_grundertrag_whg')[1] * $grundertragmultiplikator * (1 + $planertragbonus);
    $grundi = getDefaultVariable('sv_plan_grundertrag_whg')[2] * $grundertragmultiplikator * (1 + $planertragbonus);
    $grunde = getDefaultVariable('sv_plan_grundertrag_whg')[3] * $grundertragmultiplikator * (1 + $planertragbonus);

    //bonus durch spezialisierung
    $spezim = getDefaultVariable('sv_plan_grundertrag_whg')[0] * $grundertragmultiplikator * $planertragbonus;
    $spezid = getDefaultVariable('sv_plan_grundertrag_whg')[1] * $grundertragmultiplikator * $planertragbonus;
    $spezii = getDefaultVariable('sv_plan_grundertrag_whg')[2] * $grundertragmultiplikator * $planertragbonus;
    $spezie = getDefaultVariable('sv_plan_grundertrag_whg')[3] * $grundertragmultiplikator * $planertragbonus;
}


//planetarer grundertrag durch zollkontrolleure
$zollm = floor($agent_lost * getDefaultVariable('sv_zoellnerertrag')[0]);
$zolld = floor($agent_lost * getDefaultVariable('sv_zoellnerertrag')[1]);
$zolli = floor($agent_lost * getDefaultVariable('sv_zoellnerertrag')[2]);
$zolle = floor($agent_lost * getDefaultVariable('sv_zoellnerertrag')[3]);
...
///////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////
if (isset($_POST["mtr"]) or isset($_POST["dtr"]) or isset($_POST["itr"]) or isset($_POST["etr"]) or isset($_POST["ttr"])) {
    $mtr = intval($_POST["mtr"] ?? 0);
    $dtr = intval($_POST["dtr"] ?? 0);
    $itr = intval($_POST["itr"] ?? 0);
    $etr = intval($_POST["etr"] ?? 0);
    $ttr = intval($_POST["ttr"] ?? 0);
...
if (isset($fehlermsg)) echo '<div class="info_box"><span class="text2">' . $fehlermsg . '</span></div><br>';

if (isset($trademsg)) echo '<div class="info_box"><span class="text1">' . $trademsg . '</span></div><br>';

//k�nnen Kollektoren gebaut werden?
if (!hasTech($pt, 7)) {
    $techcheck = "SELECT tech_name FROM de_tech_data" . ($ums_rasse ?? 1) . " WHERE tech_id=7";
	...
	<td width="100"><a href="' . $sv_link[0] . '?r=' . $ums_rasse . '&t=7" ><img src="' . ($ums_gpfad ?? '') . 'g/t/' . $ums_rasse . '_7.jpg" border="0"></a></td>
        ...
        <table border="0" cellpadding="0" cellspacing="0" class="pctabs pctab1">
            <tr align="center">
                ...
                <td align="center" class="ro">
                    <div class="cellu"><?php echo $resource_lang['kollibau3']; ?></div>
                </td>
...
<form action="resource.php" method="POST">
    <table border="0" cellpadding="0" cellspacing="0" class="pctabs pctab2">
        <tr height="37">
            <td width="13" height="37" class="rol">&nbsp;</td>
            <td width="560" class="ro" align="center">
                <div class="cellu"><?php echo $resource_lang['resertrag'] ?></div>
            </td>
            <td width="13" class="ror">&nbsp;</td>
        </tr>
        <tr>
            ...
            <td>
                ...
                <table border="0" cellpadding="0" cellspacing="1" width="100%">
                    ...
                    if (getDefaultVariable('sv_ang') != 1) {
                        ...
                        echo '<td class="' . $bg . '" colspan=4>' . number_format($paleniumenergie, 0, "", ".") . ' (' . number_format($palenium / 100, 2, ",", ".") . ' % von max. ' . number_format(getDefaultVariable('sv_max_palenium') / 100, 2, ",", ".") . ' %)</td>';
                        echo '</tr>';
                    } else {
                        //premium-account
                        if ($_SESSION['ums_premium'] != 1) {
