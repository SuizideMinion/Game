...
include 'inc/lang/' . (getDefaultVariable('sv_server_lang') ?? 1) . '_sector.lang.php';
...
//if(getDefaultVariable('sv_bounty_rate')>getDefaultVariable('sv_kollie_klaurate'))getDefaultVariable('sv_bounty_rate')=getDefaultVariable('sv_kollie_klaurate');
...
$attgrenze = ($techs[4] == 0) ? 0:$attgrenze;
...
<head>
    ...
    function wellenrechner($kol, $maxcol, $npcsec, $sec_lang)
    {
        global $col, $ums_premium;
        ...
        if ($ums_premium == 1) {
            ...
            for ($we = 0; $we < 5; $we++) {
                ...
                if ($npcsec == 0) {
                    ...
                    $col_angriffsgrenze = $col_angriffsgrenze / 100 * getDefaultVariable('sv_max_col_attgrenze');
                    if ($col_angriffsgrenze > getDefaultVariable('sv_max_col_attgrenze')) {
                        $col_angriffsgrenze = getDefaultVariable('sv_max_col_attgrenze');
                    }
                    if ($col_angriffsgrenze < getDefaultVariable('sv_min_col_attgrenze')) {
                        $col_angriffsgrenze = getDefaultVariable('sv_min_col_attgrenze');
                    }
                } else {
                    $col_angriffsgrenze = getDefaultVariable('sv_min_col_attgrenze');
                }
                ...
                $str .= "<tr align=center><td nowrap>" . ($we + 1) . ". " . $sec_lang['welle'] . "</td><td class=" . $colclass . ">" .
                    number_format(floor($kol * getDefaultVariable('sv_kollie_klaurate')), 0, ',', '.') . "</td></tr>";
                $owncol = $owncol + floor($kol * getDefaultVariable('sv_kollie_klaurate'));
                $kol = $kol - floor($kol * getDefaultVariable('sv_kollie_klaurate'));
            }
    ...
    if ($sf > getDefaultVariable('sv_show_maxsector') or $sf < 1) {
        $sf = $ownsector;
    }
    $sector = $sf;
    ...
    if ($sec_data['npc'] == 1) {
        ...
        /*
        if($sv_oscar==1){
            getDefaultVariable('sv_min_col_attgrenze')=0.2;
        }
        */

        //kein malus bei aliens
        $sec_angriffsgrenze = getDefaultVariable('sv_attgrenze') - $attgrenze;
        $col_angriffsgrenze_final = getDefaultVariable('sv_min_col_attgrenze');
        $rec_bonus = 0;
	...
	<td width="70" class="cell tac"><font size="1">' . $sec_lang['aktion'] . '</font></td>
      ...
      <img src="' . $ums_gpfad . 'g/symbol12.png" border="0" title="Info">
        ...
        $sec_angriffsgrenze = getDefaultVariable('sv_attgrenze') - $attgrenze + $sec_malus;

        //recyclotronbonus berechnen
        $rec_bonus = getDefaultVariable('sv_recyclotron_sector_bonus') / $num * ($secplatz - 1);
        //recyclotronbonus darf nicht gr��er als das maximum sein
        if ($rec_bonus > getDefaultVariable('sv_recyclotron_sector_bonus')) $rec_bonus = getDefaultVariable('sv_recyclotron_sector_bonus');
	...
	<td width="70" class="cell tac"><font size="1">' . $sec_lang['aktion'] . '</font></td>
		...
		<span title="' . $playertooltip . '" class="' . $csstag . '">' . $playername . $osown . '</span></a></td>';
            ...
            ////////////////////////////////////////////////////////////////////////
            if ($col * $col_angriffsgrenze_final <= $row['col']) $csstag = ' text3'; else $csstag = ' text2';
            $tooltip = wellenrechner($row['col'], $maxcol, 0, $sec_lang);
        ...
        */
        ...
        if ($bed == '100') {
            if ($std > 6 and $std < 20) $bn = 'sbtag.gif'; else $bn = 'sbnacht.gif';
        } elseif ($bed == '110') {
            if ($std > 6 and $std < 20) $bn = 'sbtagsfb.gif'; else $bn = 'sbnachtsfb.gif';
        } elseif ($bed == '101') {
            if ($std > 6 and $std < 20) $bn = 'sbtagw.gif'; else $bn = 'sbnachtw.gif';
        } elseif ($bed == '111') {
            if ($std > 6 and $std < 20) $bn = 'sbtagsfbw.gif'; else $bn = 'sbnachtsfbw.gif';
        }
        //artefakte anzeigen
        ...
        if ($sector > 1 and $anz > 0) {
            ...
            //$sec_lang[relverbreitung].': '.number_format($secrelcounter*100/(getDefaultVariable('sv_maxsystem')*10), 2,",",".").'%';
