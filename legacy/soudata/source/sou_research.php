...
if($num==1)
{
  ...
  if($geb_level>0)
  {
    ...
    /////////////////////////////////////////////////
    /////////////////////////////////////////////////
    //wenn ein submit gekommen ist, dann schauen ob er rohstoffe einzahlen m�chte
    /////////////////////////////////////////////////
    /////////////////////////////////////////////////
    if($_REQUEST["do"]=='1')
    {
      ...
      {
  	    //alle forschungen durchlaufen und �berpr�fen ob es eine einzahlung gibt
  	    //for($i=0;$i<count($b_defs);$i++)
		$db_daten=mysql_query("SELECT * FROM `sou_frac_techs` ORDER BY sort_id ASC",$soudb);
  		while($row = mysql_fetch_array($db_daten))
    	{
      	  //die vorbedingungen parsen und �berpr�fen ob jemand etwas davon einzahlen m�chte
          if($row["tech_vor"]!='')
          {
            $ben_id=1;
            $i=$row["tech_id"];
            ...
            for($n=0;$n<count($need_vor);$n++)
            {
    	      ...
    	      elseif($einzelneed[0][0]=='D')
    	      {
    	      	...
    	      	//feststellen wieviel dunkle materie man ben�tigt
    	      	$need=$einzelneed[1]-$row[$search];
    	      	if($need<0)$need=0;
    	      ...
    	      /*
    	      	...
    	      	//feststellen wieviel rohstoffe man ben�tigt
		...
		if($num==1)
		{
          ...
          if($row["tech_vor"]!='')
          {
            ...
            for($n=0;$n<count($need_vor);$n++)
            {
    	      ...
    	      elseif($einzelneed[0][0]=='Z')
    	      {
	            //bei zastari nichts machen, au�er die resplatz-id zu erh�hen
    	      	$ben_id++;
    	      }
    	      //rohstoffe
    	      else
  	  			  ...
  	  			  if($freehold>0)
  	  			  {
          			  ...
          			  $has_credits=has_credits($ums_user_id);
      					...
      					if(res_is_available($einzelneed[1])==1)
      					{
          			      ...
          			      //feststellen wieviel man noch ben�tigt
    ...
    //techs anzeigen
  	...
  	while($row = mysql_fetch_array($db_daten))
    {
      //�berpr�fen ob die ben�tigte forschung vorhanden ist
      ...
      $ttid++;
	    ...
	    //wenn der erfoschen gedr�ckt wurde die forschung direkt verf�gbar machen
	    if($hasall==1 AND $_REQUEST["do"]=="build" AND $_REQUEST["bid"]==($i+1))
	    {
            //upgraden und eingezahlte res l�schen
  	  ...
  	  if($row["needenergy"]>0)
      {
  	  ...
  	  //hier die baukosten f�r das modul auflisten
      ...
      $output.='</tr>';
    }//ende in_array-test auf ben�tigte forschung
    }
    $output.='</table>';
