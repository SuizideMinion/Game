...
if($player_name=='' AND $_REQUEST["newspielername"])
{
  ...
  if(strlen($_REQUEST["newspielername"])>2 AND strlen($_REQUEST["newspielername"])<=20)
  {
    //testen ob er nur aus buchstaben besteht
  	if(!preg_match ("/^[[:alpha:]0-9������_=-]*$/i", $_REQUEST["newspielername"]))$fehlermsg.='Der Spielername ist ung�ltig.<br>';
  	else
  	{
	  ...
	  {
	  	//account anlegen
	  	mysql_query("INSERT INTO sou_user_data (ext_user_id, spielername, sn_ext1) VALUES ($ums_user_id, '$_REQUEST[newspielername]', 'getDefaultVariable('sv_server_tag')')",$soudb);
        //user_id in der session hinterlegen
	  	$_SESSION["sou_user_id"]=mysql_insert_id();

        //account mit dem de-account verkn�pfen
  ...
  else $fehlermsg.='Der Spielername hat nicht die richtige L�nge.<br>';
}
...
if($_SESSION["sou_shipname"]=='' AND $_REQUEST["newshipname"])
{
  ...
  {
    //testen ob er nur aus buchstaben besteht
  	if(!preg_match ("/^[[:alpha:]0-9������_=-]*$/i", $_REQUEST["newshipname"]))$fehlermsg.='Der Name ist ung�ltig.<br>';
  	else
  	{
  	  //schauen ob er schon vergeben ist
  ...
  else $fehlermsg.='Der Name hat nicht die richtige L�nge.<br>';
}
...
if($_SESSION["sou_fraction"]==0 AND $_REQUEST["fraction"])
{
  ...
  //gew�hlte fraktion und die startkoordinaten in die db schreiben in die db schreiben
...
if($player_name=='')
{
  ...
  - er mu� zu einem Weltraumspiel passen<br>
  - er darf nicht durch ein Copyright gesch�tzt sein<br>
  - er darf nicht rassistisch/diskriminierend/pornografisch sein<br>
  - er darf kein Stafflername sein, au�er man ist selbst der entsprechende Staffler<br><br>
  Eine Chatnutzung ist erst nach Vergabe des Spielernamens m�glich.
  </td></tr>';
...
if($_SESSION["sou_shipname"]=='')
{
  ...
  dass der Name nicht mehr ge�ndert werden kann. F�r den Namen gelten die folgende Regeln:<br>
  - erlaubt sind nur Buchstaben, Zahlen und folgende Sonderzeichen: _-=<br>
  - erlaubt sind 3-20 Buchstaben<br>
  - er mu� zu einem Weltraumspiel passen<br>
  - er darf nicht durch ein Copyright gesch�tzt sein<br>
  - er darf nicht rassistisch/diskriminierend/pornografisch sein<br>
  - er darf kein Stafflername sein, au�er man ist selbst der entsprechende Staffler<br>
  </td></tr>';
...
////////////////////////////////////////////////////////
////////////////////////////////////////////////////////
//auf fraktion/squad �berpr�fen
////////////////////////////////////////////////////////
////////////////////////////////////////////////////////
if($_SESSION["sou_fraction"]==0)
{
  //�berpr�fen ob man evtl. schon ein schiff hat und daraus die fraktion verwenden
  ...
  if($_SESSION["sou_fraction"]==0)
  {
    ...
    echo '<form action="sou_main.php" method="POST">';
    echo '<br><br><table width="600" border="0"> ';
    echo '<tr><td width="30%">Fraktion w�hlen:</td>
...
//auf destroy �berpr�fen und auf standard zur�cksetzen
if($player_destroy==1)
{
  ...
  //nachricht an den account, dass das schiff zerst�rt worden ist

  //flag f�r weiter bearbeitung setzen und durchmesser zur�cksetzen
  $player_destroy=0;
  /*
