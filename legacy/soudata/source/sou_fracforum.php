...
if($newthread)
{
	...
	elseif(!$error)
  {
    ...
    $i=1;
    $seen="s";
    while($i<getDefaultVariable('sv_maxsystem')+1)
    {
    if($i==$system)
    $seen=$seen.'1';
    else
    ...
    if($_POST[wichtig]=="1")$wichtig=1; else $wichtig=0;
    		...
    		$_POST[title] = str_replace ("ä", "&auml;", $_POST[title]);
    		$_POST[title] = str_replace ("Ä", "&Auml;", $_POST[title]);
    		$_POST[title] = str_replace ("ö", "&ouml;", $_POST[title]);
    		$_POST[title] = str_replace ("Ö", "&Ouml;", $_POST[title]);
    		$_POST[title] = str_replace ("ü", "&uuml;", $_POST[title]);
    		$_POST[title] = str_replace ("Ü", "&Uuml;", $_POST[title]);
    		$_POST[title] = str_replace ("ß", "&szlig;", $_POST[title]);

    		$_POST[title] = str_replace ("³", "&sup3;", $_POST[title]);
    		$_POST[title] = str_replace ("²", "&sup2;", $_POST[title]);
    		...
    		$_POST[nachricht] = str_replace ("³", "&sup3;", $_POST[nachricht]);
    		$_POST[nachricht] = str_replace ("²", "&sup2;", $_POST[nachricht]);
...
elseif ($a=="ota")
{
  ...
  if(!$error)
  {
    ...
    echo "OnTop f�r Thread aktiviert.";
  }else echo $error;
  }
}
...
elseif ($a=="otd")
{
  ...
  if(!$error)
  {
    ...
    echo "OnTop f�r Thread deaktiviert.";
  }else echo $error;
  }
}
...
<?
if($actionf=="edit")
echo '<input type="submit" value="�nderung speichern"  name="savedit"><input type="hidden" name="pid" value="'.$pid.'">&nbsp;<input type="submit" value="Vorschau"  name="vorschauedit"></div></td>';
else
...
if($player_user_id==$fraktionsvorsitzender_userid)
{
echo '<br><br><a href="sou_main.php?action=fracforumpage&a=c&threadid='.$id.'">Thread schlie�en</a> - <a href="sou_main.php?action=fracforumpage&a=d&threadid='.$id.'" onclick="return confirm(\'M&ouml;chten Sie diesen Thread wirklich l�schen?\')">Thread l�schen</a> - ';
...
if($player_user_id==$fraktionsvorsitzender_userid)
{
echo '<br><br><a href="sou_main.php?action=fracforumpage&a=o&threadid='.$id.'">Thread �ffnen</a> - <a href="sou_main.php?action=fracforumpage&a=d&threadid='.$id.'" onclick="return confirm(\'M&ouml;chten Sie diesen Thread wirklich l�schen?\')">Thread l�schen</a> - ';
...
echo '
   <tr>
              ...
              <input type="button" value="Gr��e"  onclick="init(\'size\')">
