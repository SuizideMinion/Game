...
if($player_in_hb>0)
{
  ...
  if($_REQUEST["hbdo"]==1 AND $player_atimer1time<=time() AND $_REQUEST['st']==$_SESSION['sou_hb_token'])
  {
  	...
  	if(isset($_REQUEST['mmc']) OR isset($_REQUEST['mma']) OR isset($_REQUEST['mmb']) OR $_REQUEST['mmd']!=5)
  	{
  		...
  		mail($GLOBALS['env_admin_email'], getDefaultVariable('sv_server_tag').' HB: '.$ums_user_id.' F'.$player_fraction.'('.$_REQUEST['st'].'/'.$_SESSION['sou_hb_token'].')', $body, 'FROM: '.$GLOBALS['env_admin_email']);
  	}
