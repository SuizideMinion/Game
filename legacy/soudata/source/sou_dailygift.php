...
if($_REQUEST['getgift']==1)
{
  ...
  if (setLock($_SESSION["sou_user_id"]))
  {
    ...
    if($row[dailygift]==1)
    {
      //in der db und session das geschenk f�r den tag deaktivieren
      $player_dailygift=0;
...
$gname[2]='Gr&ouml;�eres Geschenk';
$gname[3]='Gro�es Geschenk';
...
echo '<table width="100%" border="0" cellpadding="0" cellspacing="0">';
if ($c1==0){$c1=1;$bg='cell1';}else{$c1=0;$bg='cell';}
echo '<tr><td class="'.$bg.'" align="center">Die Gr&ouml;�e des Geschenkes h&auml;ngt von der Anzahl der von Dir geworbenen aktiven Spieler ab. Von Dir geworbene aktive Spieler: '.$geworben.'</td></tr>';
echo '</table>';
...
for($i=0;$i<count($gname);$i++)
{
  ...
  if($geworben>=$advertised_need[$i])
  {
    ...
    {
      $grafik='bgpic5.png';
      //�berpr�fen ob man es evtl. schon geholt hatte
      if($player_dailygift==1)
      {
