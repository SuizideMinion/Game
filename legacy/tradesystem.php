    ...
    if($num<13)// es sind welche notwendig
    {
	    ...
	    for($i=0;$i<count($tech_ids);$i++)
	    {
	    	...
	    	if($vorhanden[$tech_id]!=1)
	    	{
		    	...
		    	$uwatt=($maxtick-$lastpcatt)*100/(getDefaultVariable('sv_winscore')/4)/4;
		    	...
		    	*/
				...
				if(getDefaultVariable('sv_ewige_runde')==1 && $maxtick > getDefaultVariable('sv_winscore')){
					//$maxtrades=getDefaultVariable('sv_winscore')/24*4;
				}
		    	//durch 4 teilen um auf den 25er-anteil zu kommen, 100% sind 25 einheiten
