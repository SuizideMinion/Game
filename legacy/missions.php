		...
		if($vorbedingungen_erfuellt){
			...
			if($md[$m]['typ']==0 || $md[$m]['typ']==1 || $md[$m]['typ']==2){
					...
					if($pd['agent']>=$md[$m]['need_agents']){
						...
						if($storage_is_ok){
							...
							if($um[$m]['end_time']<time() && ($um[$m]['get_reward']==1 || $um[$m]['get_reward']=='')){
								...
								$end_time=round(time()+$md[$m]['time']*getDefaultVariable('tech_build_time_faktor'));
								$sql="
								...
								if($md[$m]['typ']==1 || $md[$m]['typ']==2){
									//Handelspunkte
									$tradescore=$md[$m]['time']*getDefaultVariable('tech_build_time_faktor')*$reward_percentage/100;
				...
				if(!isset($um[$m]) || ($um[$m]['end_time']<time() && $um[$m]['get_reward']==1)){
					$content.=' (Dauer: <span id="mission_counter'.$m.'">'.formatTime($md[$m]['time']*getDefaultVariable('tech_build_time_faktor')).'</span>)';
				}else{
				...
				if($md[$m]['typ']==1){
					$tradescore=$md[$m]['time']*getDefaultVariable('tech_build_time_faktor');
				...
				if(!isset($um[$m]) || ($um[$m]['end_time']<time() && $um[$m]['get_reward']==1)){
					if($md[$m]['typ']==0){
						...
						for($fleet_id=1;$fleet_id<=3;$fleet_id++){
							...
							$tradescore=$md[$m]['time']*getDefaultVariable('tech_build_time_faktor')*$p/100;
							//$belohnung_tradescore=number_format($tradescore, 0,"",".").' Handelspunkte';
							$belohnung_tradescore='<img src="g/icon11.png" style="height: 20px; width: auto; margin-bottom: -5px;" rel="tooltip" title="Handelspunkte"> <div style="display: inline-block; margin-bottom: 5px;">'.number_format($tradescore, 0,"",".").'</div><br>';
