...
if(setLock($ums_user_id)){
	...
	if(isset($_REQUEST['cancel_tech']) && $_REQUEST['cancel_tech']>0){
		...
		if(isset($pt[$tech_id]) && $pt[$tech_id]['time_finished']>time()){
			...
			//
			$time_finished=time()+floor($tech_daten[$tech_id]['tech_build_time']*getDefaultVariable('tech_build_time_faktor'));
		...
		if($max_wt>1){
			...
			if($active_tech_types[$tech_typ]==0){
				...
				if(!isset($pt[$tech_id])){
					...
					if(getDefaultVariable('sv_deactivate_vsystems')==1 && $tech_typ==3){
						$has_all=false;
					}
	...
	while($row = mysqli_fetch_array($db_daten)){
		...
		$bauzeit='<br><br><span style=\'color: rgba(255,255,255, 0.4)\'>Dauer: '.formatTime($row['tech_build_time']*getDefaultVariable('tech_build_time_faktor')).'</span>';
		...
		//wenn die V-Systeme deaktiviert sind, dann sind die Techs auch nicht nutzbar
		if(getDefaultVariable('sv_deactivate_vsystems')==1 && $row['tech_typ']==3){
			$hide_tech=true;
		}
