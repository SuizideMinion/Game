<?php
include '../inc/lang/'.getDefaultVariable('sv_server_lang').'_trade.tick.lang.php';

function doTradeTick()
{
	global $tradetick_lang;
	global getDefaultVariable('sv_server_lang');
	global getDefaultVariable('sv_deactivate_trade');
	...
	*/
	...
	//handelseintr�ge um eins zur�cksetzen
	...
	//abgelaufene Ressourcen zur�ckbuchen
	...
	//abgelaufene Flotten zur�ckbuchen
	...
	//in Lieferung befindliche Schiffe um eins zur�cksetzen
						...
						if ($result)
						{
							...
							if ($offer_numrows == 1)
							{
								...
								if ($request_amount == $offer_amount)
								{
									...
									$request_amount = 0;
									$offer_amount = 0;
									//Angebot und request aus DB l�schen, beide spieler benachrichtigen und Daten aktualisieren
									//berechnen, was k�ufer bezahlen muss
									$buyer_to_pay = $booked_amount * $offer_price;
									//berechnen was k�ufer geboten hat
									$buyer_max_pay = $booked_amount * $request_price;
									//berechnen, was er wegen eines ev. g�nstigeren gebots zur�ckbekommt
								...
								elseif($request_amount > $offer_amount)
								{
									...
									$offer_amount = 0;
									//Angebot aus DB l�schen, beide spieler benachrichtigen und Daten aktualisieren
								...
								elseif ($request_amount < $offer_amount)
								{
									...
									//request aus DB l�schen, beide spieler benachrichtigen und Daten aktualisieren
									//Angebot und request aus DB l�schen, beide spieler benachrichtigen und Daten aktualisieren
									//berechnen, was k�ufer bezahlen muss
									$buyer_to_pay = $booked_amount * $offer_price;
									//berechnen was k�ufer geboten hat
						...
						if ($result)
						{
							...
							if ($offer_numrows == 1)
							{
								...
								if ($request_amount == $offer_amount)
								{
									...
									$request_amount = 0;
									$offer_amount = 0;
									//Angebot und request aus DB l�schen, beide spieler benachrichtigen und Ress aktualisieren
									//berechnen, was k�ufer bezahlen muss
									$buyer_to_pay = $booked_amount * $offer_price;
									//berechnen was k�ufer geboten hat
									$buyer_max_pay = $booked_amount * $request_price;
								...
								elseif($request_amount > $offer_amount)
								{
									...
									$request_amount = $request_amount - $booked_amount;
									$offer_amount = 0;
									//Angebot aus DB l�schen, beide spieler benachrichtigen und ress aktualisieren
									//print("$booked_amount Units booked. Still remaining $request_amount amount units of $request_buy_type to book<br>");
									//Angebot und request aus DB l�schen, beide spieler benachrichtigen und Ress aktualisieren
									//berechnen, was k�ufer bezahlen muss
									$buyer_to_pay = $booked_amount * $offer_price;
								...
								elseif ($request_amount < $offer_amount)
								{
									...
									$request_amount = 0;
									$offer_amount = $offer_amount - $booked_amount;
									//request aus DB l�schen, beide spieler benachrichtigen und ress aktualisieren
									//Angebot und request aus DB l�schen, beide spieler benachrichtigen und Ress aktualisieren
									//berechnen, was k�ufer bezahlen muss
									$buyer_to_pay = $booked_amount * $offer_price;
									//berechnen was k�ufer geboten hat
...
function bookFleetTrade($request_user_id, $booked_amount, $request_buy_type, $buyer_payback, $request_currency, $offer_user_id, $buyer_to_pay, $offer_race, $offer_ssatz, $offer_sector, $offer_price, $request_price, $fromdepot)
{
	...
	//Erl�s vor Steuerabzug (unbereinigter Gewinn)
	...
	//print("Echte Werte:<br>Echter Schiffswert: $book_ship_real_value $currency_displayname<br>Erzielter Schiffswert: $book_ship_sold_value $currency_displayname<br>Erzielter Erl�s: $book_trade_value $currency_displayname<br>Erl�s nach Steuer ($offer_ssatz %): $book_trade_resultvalue<br>Gewinn nach Steuer ($offer_ssatz %): $book_trade_result_profit<br>Bezahlte Steuer: $book_tax<br>Zu bezahlender Betrag: $book_buyer_to_pay $currency_displayname<br>Zu erstattender Betrag: $book_buyer_payback $currency_displayname<br><hr>");
