...
echo '<br> Da die Logdatenbank sehr gro� ist, kann die Suche sehr lange dauern. Einfach den gesuchten Text eingeben und mit Return/Enter best&auml;tigen.';
...
if($_REQUEST['searchtext'])
{
	...
	if (!$dblog)
	{
  		echo 'keine Verbindung m�glich: ' . mysql_error();
	}
	mysql_select_db($targetdata[3], $dblog);

	echo '<table>';
	echo '<tr><td>Zeit</td><td>IP</td><td>Datei</td><td>getpost</td></tr>';
	//gew�nschten text suchen
	$db_daten=mysql_query("SELECT *  FROM ".$targetdata[4]." WHERE userid='$uid' AND serverid='getDefaultVariable('sv_servid')' AND getpost LIKE '%".mysql_real_escape_string($_REQUEST['searchtext'])."%';",$dblog);
