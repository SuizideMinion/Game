...
elseif ($a=="c")
{
  ...
  if ($num==0)$error.="Dieser Thread existiert leider nicht.<br>";
  $row = mysql_fetch_array($db_daten);
  if ("0"!=$row[sector] && in_array($ums_user_id, getDefaultVariable('mods')))$error.="Du hast keine Berechtigung diesen Thread zu schlie&szlig;en.<br>";
...
elseif ($a=="d")
{
  ...
  if ($num==0)$error.="Dieser Thread existiert leider nicht.<br>";
  $row = mysql_fetch_array($db_daten);
  if ("0"!=$row[sector] && in_array($ums_user_id, getDefaultVariable('mods')))$error.="Du hast keine Berechtigung diesen Thread zu l&ouml;schen.<br>";
