...
include('inc/lang/' . getDefaultVariable('sv_server_lang') . '_options.lang.php');
    ...
    if ($passwordOK) { //oldpass ist korrekt
        if ($delcheck1 == "1" and $delcheck2 == "1") {//l�sche
            ...
            if ($num == 0) {//man ist kein leader
                ...
                /*
                $deltage=getDefaultVariable('sv_inactiv_deldays')-1;
                $uid=$ums_user_id;
                ...
                */
                ...
                $newtick = $maxtick + (getDefaultVariable('sv_benticks') * $ehlockfaktor);
                ...
                @mail_smtp($reg_mail, $options_lang['emailgeloeschtbetreff'] . ' - ' . getDefaultVariable('sv_server_name'), $options_lang['emailgeloeschtbody'], 'FROM: noreply@die-ewigen.com');
...
//Logout anzeige
$sekundenbiszumlogout = ($ums_session_start + getDefaultVariable('sv_session_lifetime')) - time();
...
if (isset($_COOKIE['deactivate_swipe']) AND $_COOKIE['deactivate_swipe'] == 0) {
...
<tr align="center">
<td width="13" height="25" class="rl">&nbsp;</td>
<td><?=$options_lang[accountid]?>: <?=getDefaultVariable('sv_server_tag').$ums_user_id?></td>
<td width="13" class="rr">&nbsp;</td>
</tr>
</form>
*/
...
echo '<br><font color="#FFFF00">' . $options_lang['accountloescheninfo3'] . ' ' . number_format(getDefaultVariable('sv_benticks') * $ehlockfaktor, 0, "", ".") . '</font>';
