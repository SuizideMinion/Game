...
$ac_prozent=getDefaultVariable('sv_winscore')/960/100*2.75;
...
//besitze kollektoren
$achievement_anz++;
$stufen=round(getDefaultVariable('sv_winscore')/1440);
...
for($index=0;$index<$stufen;$index++){
	...
	if(getDefaultVariable('sv_deactivate_efta')==0)$max_achievement_points++;
}
...
for($index=0;$index<$stufen;$index++){
	...
	if(getDefaultVariable('sv_deactivate_sou')==0)$max_achievement_points++;
}
...
for($index=0;$index<$stufen;$index++)
{
	$rewards14[$index][0]=floor(($index+1)*800000*getDefaultVariable('tech_build_time_faktor')); //zu erreichender wert
	@$rewards14[$index][1]=round(($mbelohnung/($stufen+1))*($index+1)/($stufen/2)); //belohnung in M
	if(getDefaultVariable('sv_deactivate_trade')==0)$max_achievement_points++;
}
