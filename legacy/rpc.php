...
include "inc/lang/".getDefaultVariable('sv_server_lang')."_rpc.lang.php";
...
if(isset($_REQUEST["getaccountdata"]) && $_REQUEST["getaccountdata"]==1)
{
  ...
  if($num==1){
    ...
    //spielerbeschreibung
    echo ';';
    echo $rpc_lang['serverspielerid'].': '.getDefaultVariable('sv_server_tag').$user_id.'<br>';
    echo $rpc_lang['spielername'].': '.urlencode($spielername).'<br>';
    if(getDefaultVariable('sv_sou_in_de')!=0 AND getDefaultVariable('sv_efta_in_de')!=0)echo $rpc_lang['punkte'].': '.number_format($row["score"], 0,"",".").'<br>';
    if(getDefaultVariable('sv_sou_in_de')!=0 AND getDefaultVariable('sv_efta_in_de')!=0)echo $rpc_lang['accountalter'].': '.number_format($row["tick"], 0,"",".").'<br>';
    if(getDefaultVariable('sv_sou_in_de')!=0 AND getDefaultVariable('sv_efta_in_de')!=0)echo $rpc_lang['allianz'].': '.urlencode($allytag).'<br>';
...
if(isset($_REQUEST["createaccount"]) && $_REQUEST["createaccount"]==1){
  ...
  //if(getDefaultVariable('sv_efta_in_de')==1 AND getDefaultVariable('sv_sou_in_de')==1)$status=0;else $status=1;
  ...
  if(getDefaultVariable('sv_efta_in_de')==1 AND getDefaultVariable('sv_sou_in_de')==1)
  {
    ...
    if(getDefaultVariable('sv_ewige_runde')!=1){
      ...
      if($maxtick>1){
        ...
        //rohstoffe berechnen
        $m=round(getDefaultVariable('sv_plan_grundertrag')[0]*($maxtick-1)/5);
        $d=round(getDefaultVariable('sv_plan_grundertrag')[1]*($maxtick-1)/5);
